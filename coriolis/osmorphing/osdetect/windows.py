    ...
    def _get_image_version_info(self):
        ...
        try:
            version_info_str = self._conn.exec_ps_command(
                ...
                "InstallationType, ProductName, EditionID | FL" % key_name
            ).replace(self._conn.EOL, os.linesep)
        finally:
        ...
        if (
            not version_info["CurrentMajorVersionNumber"]
            and not version_info["CurrentVersion"]
        ):
            raise exception.CoriolisException("Cannot find Windows version info")
