...
class LinuxOSDetectUsingOSRelease(BaseLinuxOSDetectTools):
    ...
    def detect_os(self):
        ...
        if not distribution_name:
            LOG.warning("Could not detect OS from /etc/os-release: NAME is missing")
            return {}

        version = os_release.get("VERSION_ID")
        if not version:
            LOG.warning(
                "Could not detect OS from /etc/os-release: VERSION_ID is missing"
            )
            return {}
        ...
        return {
            ...
            "friendly_release_name": "%s Version %s" % (distribution_name, version),
        }
