...
from coriolis.osmorphing.osdetect import base, coreos, debian, openwrt, suse, windows
...
def detect_os(
    ...
    for detectcls in detect_tools_classes:
        ...
        if detected_info:
            LOG.info(
                "Successfully detected OS using tools '%s'. Returned OS info is: %s",
                detectcls.__name__,
                detected_info,
            )
            break
    ...
    extra_returns = [field for field in detected_info if field not in declared_returns]
    if extra_returns:
        raise exception.InvalidDetectedOSParams(
