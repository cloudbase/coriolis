...
class OracleOSDetectTools(base.BaseLinuxOSDetectTools):
    def detect_os(self):
        ...
        if self._test_path(oracle_release_path):
            release_info = self._read_file(oracle_release_path).decode().splitlines()
            if release_info:
                m = re.match(r"^(.*) release ([0-9].*)$", release_info[0].strip())
                if m:
                    ...
                    info = {
                        ...
                        "friendly_release_name": "%s Version %s" % (distro, version),
                    }
        return info
