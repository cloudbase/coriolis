...
class BaseUbuntuMorphingTools(debian.BaseDebianMorphingTools):
    ...
    def check_os_supported(cls, detected_os_info):
        if detected_os_info['distribution_name'] != (UBUNTU_DISTRO_IDENTIFIER):
            return False
    ...
    def _set_netplan_ethernet_configs(
        ...
        for conffile in self._list_dir("etc/netplan"):
            ...
            config_data = yaml.load(
                io.StringIO(config_contents.decode("utf-8")), Loader=yaml.SafeLoader
            )
            ...
            if not ethernet_configurations:
                LOG.debug("No Ethernet configurations in file %s", config_path_chroot)
            new_ethernet_configurations = {}
            set_dhcp = bool(dhcp)
            for iface_name in ethernet_configurations.keys():
                new_iface_name = iface_name
                if iface_name_prefix and not iface_name.startswith(iface_name_prefix):
                    if iface_name in processed_interfaces.keys():
                        ...
                        new_iface_name = "%s%d" % (iface_name_prefix, interface_index)
