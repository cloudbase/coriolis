...
class InterfacesNetPreserver(base.BaseNetPreserver):
    ...
    def check_net_preserver(self):
        if self.osmorphing_tool._test_path(self.ifaces_file):
            if self.osmorphing_tool._list_dir(os.path.dirname(self.ifaces_file)):
                return True
        return False
    ...
    def parse_network(self):
        ...
        def _parse_iface_file(interface_file):
            ...
            curr_iface = None
            interfaces_contents = self.osmorphing_tool._read_file_sudo(interface_file)
            LOG.debug("Fetched %s contents: %s", interface_file, interfaces_contents)
            for line in interfaces_contents.splitlines():
                if line.strip().startswith("iface"):
                    ...
                    if len(words) > 2:
                        if not curr_iface:
                            LOG.warn(
                                "Found MAC address %s does not belong to "
                                "any interface stanza. Skipping.",
                                words[2],
                            )
                            continue

                        self.interface_info[curr_iface]["mac_address"] = words[2]
                elif line.strip().startswith("address"):
                    words = line.split()
                    if len(words) > 1:
                        curr_ip_address = words[1].split('/')[0]
                        if curr_iface:
                            self.interface_info[curr_iface]["ip_addresses"].append(
                                curr_ip_address
                            )
