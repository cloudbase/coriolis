...
class NetplanNetPreserver(base.BaseNetPreserver):
    ...
    def check_net_preserver(self):
        if self.osmorphing_tool._test_path(self.netplan_base):
            return any(
                f.endswith((".yaml", ".yml"))
                for f in self.osmorphing_tool._list_dir(self.netplan_base)
            )
        return False
        ...
        for cfg in self.netplan_files:
            ...
            try:
                contents = yaml.safe_load(
                    self.osmorphing_tool._read_file_sudo(cfg_path)
                )
                ...
                for iface, net_cfg in ifaces.items():
                    self.interface_info[iface] = {"mac_address": "", "ip_addresses": []}
                    ...
                    for ip in net_cfg.get('addresses', []):
                        if isinstance(ip, dict):
                            ...
                            if not ip_keys:
                                LOG.warning(
                                    "Found empty IP address object entry. Skipping"
                                )
                                continue
                            ip_obj_addrs = [addr.split('/')[0] for addr in ip_keys]
                ...
                LOG.warn(
                    "Could not parse netplan configuration '%s'. Invalid YAML file: %s",
                    cfg_path,
