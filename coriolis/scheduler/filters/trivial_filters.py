...
class RegionsFilter(base.BaseServiceFilter):
    ...
    def rate_service(self, service):
        if not self._regions:
            LOG.debug(
                "No regions specified for this filter (%s). Presuming service is valid."
            )
            return 100

        service_regions = [region.id for region in service.mapped_regions]
        found = []
        missing = []
        ...
        if not found:
            LOG.debug(
                "None of the requested regions are available on service (%s): %s",
                service.id,
                self._regions,
            )
            return 0
...
class TopicFilter(base.BaseServiceFilter):
    ...
    def __repr__(self):
        return "<%s(topic=%s)>" % (self.__class__.__name__, self._topic)
...
class EnabledFilter(base.BaseServiceFilter):
    ...
    def __repr__(self):
        return "<%s(enabled=%s)>" % (self.__class__.__name__, self._enabled)
...
class ProviderTypesFilter(base.BaseServiceFilter):
    ...
    def rate_service(self, service):
        for platform_type in self._provider_requirements:
            ...
            available_types = service.providers[platform_type].get('types', [])
            missing_types = [
                typ
                for typ in self._provider_requirements[platform_type]
