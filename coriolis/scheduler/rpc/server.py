...
class SchedulerServerEndpoint(object):
    ...
    def _get_weighted_filtered_services(
        ...
        """
        if not filters:
            LOG.warn("No filters provided. Presuming all services acceptable.")
        ...
        for service in services:
            ...
            if not acceptable:
                LOG.debug(
                    "Service with ID '%s' was rejected by filter %r", service.id, flt
                )
                continue
        ...
        if not scores:
            message = (
                ...
                % ([s.id for s in services], minimum_per_filter_rating, filters)
            )
            raise exception.NoSuitableWorkerServiceError(message)

        LOG.debug(
            "Determined following scores for services based on filters '%s': %s",
            filters,
            scores,
        )
    ...
    def _filter_regions(
        self, ctxt, region_ids, enabled=True, check_all_exist=True, regions_cache=None
    ):
        found_regions = []
        filtered_regions = []
        ...
        if check_all_exist:
            missing_regions = set(region_ids).difference(set(found_regions))
            if missing_regions:
    ...
    def get_workers_for_specs(
        ...
        if region_sets:
            for region_set in region_sets:
                ...
                filtered_regions = self._filter_regions(
                    ctxt,
                    region_set,
                    enabled=filter_disabled_regions,
        ...
        if provider_requirements:
            filters.append(trivial_filters.ProviderTypesFilter(provider_requirements))
