...
from coriolis import constants, exception, rpc, utils
...
scheduler_opts = [
    cfg.IntOpt(
        "scheduler_rpc_timeout",
        help="Number of seconds until RPC calls to the scheduler timeout.",
    )
]
...
class SchedulerClient(rpc.BaseRPCClient):
    def __init__(self, timeout=None):
        ...
        super(SchedulerClient, self).__init__(target, timeout=timeout)

    def get_diagnostics(self, ctxt):
        return self._call(ctxt, 'get_diagnostics')

    def get_workers_for_specs(
        self, ctxt, provider_requirements=None, region_sets=None, enabled=None
    ):
        return self._call(
            ctxt,
    ...
    def get_any_worker_service(self, ctxt, random_choice=False, raise_if_none=True):
        ...
        LOG.debug(
            "Selected service with ID '%s' for any-worker request.", service['id']
        )
        return service
    ...
    def get_worker_service_for_specs(
        ...
        """
        requirements_str = "enabled=%s; region_sets=%s; provider_requirements=%s" % (
            enabled,
            region_sets,
    ...
    def get_worker_service_for_task(
        ...
        if constants.PROVIDER_PLATFORM_SOURCE in required_provider_types:
            provider_requirements[origin_endpoint['type']] = required_provider_types[
        ...
        if constants.PROVIDER_PLATFORM_DESTINATION in required_provider_types:
            provider_requirements[destination_endpoint['type']] = (
                required_provider_types[constants.PROVIDER_PLATFORM_DESTINATION]
            )
        ...
        for i in range(retry_count):
            try:
                ...
                worker_service = self.get_worker_service_for_specs(
                    ctxt,
                    provider_requirements=provider_requirements,
                    region_sets=required_region_sets,
                    enabled=True,
