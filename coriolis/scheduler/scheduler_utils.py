...
RPC_TOPIC_TO_CLIENT_CLASS_MAP = {
    ...
    constants.SCHEDULER_MAIN_MESSAGING_TOPIC: (rpc_scheduler_client.SchedulerClient),
    constants.TRANSFER_CRON_MAIN_MESSAGING_TOPIC: (rpc_cron_client.TransferCronClient),
}
...
def get_rpc_client_for_service(service, *client_args, **client_kwargs):
    ...
    if not rpc_client_class:
        raise exception.NotFound(
            "No RPC client class for service with topic '%s'." % (service.topic)
        )
    ...
    if service.topic == constants.WORKER_MAIN_MESSAGING_TOPIC:
        ...
        topic = constants.SERVICE_MESSAGING_TOPIC_FORMAT % (
            {"main_topic": constants.WORKER_MAIN_MESSAGING_TOPIC, "host": service.host}
        )

    return rpc_client_class(*client_args, topic=topic, **client_kwargs)


def get_any_worker_service(scheduler_client, ctxt, random_choice=False, raw_dict=False):
...
def get_worker_rpc_for_host(host, *client_args, **client_kwargs):
    ...
    topic = constants.SERVICE_MESSAGING_TOPIC_FORMAT % (
        {"main_topic": constants.WORKER_MAIN_MESSAGING_TOPIC, "host": host}
    )
    return rpc_client_class(*client_args, topic=topic, **client_kwargs)
