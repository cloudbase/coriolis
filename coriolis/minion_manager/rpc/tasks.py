...
MINION_POOL_MIGRATION_ALLOCATION_FLOW_NAME_FORMAT = "migration-%s-minions-allocation"
MINION_POOL_REPLICA_ALLOCATION_FLOW_NAME_FORMAT = "replica-%s-minions-allocation"
...
MINION_POOL_ALLOCATE_MINIONS_SUBFLOW_NAME_FORMAT = "pool-%s-machines-allocation"
MINION_POOL_DEALLOCATE_MACHINES_SUBFLOW_NAME_FORMAT = "pool-%s-machines-deallocation"
MINION_POOL_HEALTHCHECK_MACHINE_SUBFLOW_NAME_FORMAT = "pool-%s-machine-%s-healthcheck"
MINION_POOL_REALLOCATE_MACHINE_SUBFLOW_NAME_FORMAT = "pool-%s-machine-%s-reallocation"
...
MINION_POOL_ALLOCATE_MACHINE_TASK_NAME_FORMAT = "pool-%s-machine-%s-allocation"
MINION_POOL_DEALLOCATE_MACHINE_TASK_NAME_FORMAT = "pool-%s-machine-%s-deallocation"
...
MINION_POOL_POWER_ON_MACHINE_TASK_NAME_FORMAT = "pool-%s-machine-%s-power-on"
MINION_POOL_POWER_OFF_MACHINE_TASK_NAME_FORMAT = "pool-%s-machine-%s-power-off"
...
class MinionManagerTaskEventMixin(object):
    ...
    def _add_minion_pool_event(self, context, message, level=constants.TASK_EVENT_INFO):
        LOG.debug("Minion pool '%s' event: %s", self._minion_pool_id, message)
        db_api.add_minion_pool_event(context, self._minion_pool_id, level, message)

    def _get_minion_machine(self, context, minion_machine_id, raise_if_not_found=False):
        machine = db_api.get_minion_machine(context, minion_machine_id)
        if not machine and raise_if_not_found:
            raise exception.NotFound(
                "Could not find minion machine with ID '%s'" % (minion_machine_id)
            )
        return machine

    def _set_minion_pool_status(self, ctxt, minion_pool_id, new_status):
        with minion_manager_utils.get_minion_pool_lock(minion_pool_id, external=True):
    ...
    def _update_minion_machine(
        ...
        with minion_manager_utils.get_minion_pool_lock(minion_pool_id, external=True):
            db_api.update_minion_machine(ctxt, minion_machine_id, updated_values)
    ...
    def _set_minion_machine_power_status(
        ...
        self._update_minion_machine(
            ctxt, minion_pool_id, minion_machine_id, {"power_status": new_status}
        )
...
class _BaseReportMinionAllocationFailureForActionTask(
    ...
    def _get_task_name(self, action_id):
        raise NotImplementedError("No allocation failure task name provided")

    @abc.abstractmethod
    def _report_machine_allocation_failure(self, context, action_id, failure_str):
        raise NotImplementedError("No allocation failure operation defined")

    def execute(self, context):
        super(_BaseReportMinionAllocationFailureForActionTask, self).execute()
        LOG.debug("Nothing to execute for task '%s'", self._task_name)

    def revert(self, context, *args, **kwargs):
        super(_BaseReportMinionAllocationFailureForActionTask, self).revert(
            *args, **kwargs
        )
...
class _BaseConfirmMinionAllocationForActionTask(
    ...
    @abc.abstractmethod
    def _get_task_name(self, action_id):
        raise NotImplementedError("No minion allocation confirmation task name defined")

    @abc.abstractmethod
    def _confirm_machine_allocation_for_action(
        self, context, action_id, machine_allocations
    ):
        raise NotImplementedError("No minion allocation confirmation operation defined")
    ...
    def execute(self, context):
        ...
        for (
            instance,
            allocated_machines_for_instance,
        ) in self._allocated_machine_id_mappings.items():
            ...
            origin_minion_id = allocated_machines_for_instance.get('origin_minion_id')
            if origin_minion_id:
            ...
            if destination_minion_id:
                destination_minion_machine = machines_cache.get(destination_minion_id)
                if not destination_minion_machine:
                    destination_minion_machine = self._get_minion_machine(
                        context, destination_minion_id, raise_if_not_found=True
                    )
                    _check_minion_properties(
                    ...
                    machines_cache[destination_minion_id] = destination_minion_machine
            ...
            if osmorphing_minion_id:
                osmorphing_minion_machine = machines_cache.get(osmorphing_minion_id)
                if not osmorphing_minion_machine:
                    ...
                    _check_minion_properties(
                        osmorphing_minion_machine, instance, minion_purpose="OSMorphing"
                    )
                    machines_cache[osmorphing_minion_id] = osmorphing_minion_machine
        ...
        try:
            ...
            raise exception.MinionMachineAllocationFailure(msg) from ex
        except (exception.InvalidDeploymentState, exception.InvalidTransferState) as ex:
            msg = (
                "The Conductor has refused minion machine allocations for "
...
class ConfirmMinionAllocationForReplicaTask(_BaseConfirmMinionAllocationForActionTask):
    def _get_action_label(self):
        return "replica"
...
class UpdateMinionPoolStatusTask(
    ...
    def __init__(
        self, minion_pool_id, target_status, status_to_revert_to=None, **kwargs
    ):

        self._target_status = target_status
        self._minion_pool_id = minion_pool_id
        self._task_name = (
            MINION_POOL_UPDATE_STATUS_TASK_NAME_FORMAT
            % (self._minion_pool_id, self._target_status)
        ).lower()
        self._previous_status = None
        self._status_to_revert_to = status_to_revert_to

        super(UpdateMinionPoolStatusTask, self).__init__(name=self._task_name, **kwargs)
    ...
    def execute(self, context, *args):
        ...
        if self._previous_status == self._target_status:
            LOG.debug(
                ...
                "Nothing to do."
                % (self._task_name, self._minion_pool_id, self._target_status)
            )
        else:
            LOG.debug(
    ...
    def revert(self, context, *args, **kwargs):
        ...
        if minion_pool.status == previous_status:
            LOG.debug(
                ...
                "'%s'." % (self._task_name, self._minion_pool_id, previous_status)
            )
        else:
            ...
            self._set_minion_pool_status(context, self._minion_pool_id, previous_status)
            self._add_minion_pool_event(
                context,
...
class BaseMinionManangerTask(
    coriolis_taskflow_base.BaseRunWorkerTask, MinionManagerTaskEventMixin
):
    ...
    def __init__(
        self, minion_pool_id, minion_machine_id, main_task_runner_type, **kwargs
    ):
        self._minion_pool_id = minion_pool_id
...
class ValidateMinionPoolOptionsTask(BaseMinionManangerTask):
    def __init__(self, minion_pool_id, minion_machine_id, minion_pool_type, **kwargs):
        task_type = constants.TASK_TYPE_VALIDATE_SOURCE_MINION_POOL_OPTIONS
        if minion_pool_type != constants.PROVIDER_PLATFORM_SOURCE:
            task_type = constants.TASK_TYPE_VALIDATE_DESTINATION_MINION_POOL_OPTIONS
    ...
    def execute(self, context, origin, destination, task_info):
        self._add_minion_pool_event(context, "Validating minion pool options")
...
class AllocateSharedPoolResourcesTask(BaseMinionManangerTask):
    ...
    def _get_task_name(self, minion_pool_id, minion_machine_id):
        return MINION_POOL_ALLOCATE_SHARED_RESOURCES_TASK_NAME_FORMAT % (minion_pool_id)

    def execute(self, context, origin, destination, task_info):
        with minion_manager_utils.get_minion_pool_lock(
            self._minion_pool_id, external=True
        ):
            minion_pool = db_api.get_minion_pool(context, self._minion_pool_id)
            if not minion_pool:
            ...
            if minion_pool.shared_resources:
                raise exception.InvalidMinionPoolState(
                    ...
                    "DB entry is: %s" % (self._task_name, minion_pool.shared_resources)
                )

        self._add_minion_pool_event(context, "Deploying shared pool resources")
        res = super(AllocateSharedPoolResourcesTask, self).execute(
            context, origin, destination, task_info
        )
        pool_shared_resources = res['pool_shared_resources']

        updated_values = {"shared_resources": pool_shared_resources}
        ...
        with minion_manager_utils.get_minion_pool_lock(
            ...
            db_api.update_minion_pool(context, self._minion_pool_id, updated_values)
    ...
    def revert(self, context, origin, destination, task_info, **kwargs):
        ...
        with minion_manager_utils.get_minion_pool_lock(
            ...
            updated_values = {"pool_shared_resources": None}
            db_api.update_minion_pool(context, self._minion_pool_id, updated_values)
...
class DeallocateSharedPoolResourcesTask(BaseMinionManangerTask):
    def __init__(self, minion_pool_id, minion_machine_id, minion_pool_type, **kwargs):
        ...
        super(DeallocateSharedPoolResourcesTask, self).__init__(
            minion_pool_id, minion_machine_id, resource_deallocation_task, **kwargs
        )
    ...
    def execute(self, context, origin, destination, task_info):
        self._add_minion_pool_event(context, "Deallocating shared pool resources")
        if 'pool_shared_resources' not in task_info:
        ...
        execution_info = {
            "pool_environment_options": task_info.get('pool_environment_options', {}),
        ...
        updated_values = {"shared_resources": None}
        db_api.update_minion_pool(context, self._minion_pool_id, updated_values)
        self._add_minion_pool_event(
...
class AllocateMinionMachineTask(BaseMinionManangerTask):
    def __init__(
        ...
        **kwargs,
        ...
        resource_deployment_task_type = constants.TASK_TYPE_CREATE_SOURCE_MINION_MACHINE
        resource_cleanup_task_type = constants.TASK_TYPE_DELETE_SOURCE_MINION_MACHINE
        if minion_pool_type != constants.PROVIDER_PLATFORM_SOURCE:
            resource_deployment_task_type = (
        ...
        super(AllocateMinionMachineTask, self).__init__(
            ...
            resource_deployment_task_type,
            cleanup_task_runner_type=resource_cleanup_task_type,
            raise_on_cleanup_failure=raise_on_cleanup_failure,
            **kwargs,
        )
    ...
    def execute(self, context, origin, destination, task_info):
        ...
        if minion_machine:
            ...
            if self._allocate_to_action and (
                minion_machine.allocated_action
                and (self._allocate_to_action != (minion_machine.allocated_action))
            ):
                raise exception.InvalidMinionMachineState(
            ...
            log_msg = "[Task '%s'] Adding new minion machine with ID '%s' to the DB" % (
                self._task_name,
                self._minion_machine_id,
            )
        ...
        event_message = "Allocating minion machine with internal pool ID '%s'" % (
            self._minion_machine_id
        )
        if self._allocate_to_action:
            event_message = "%s to be used for transfer action with ID '%s'" % (
                event_message,
                self._allocate_to_action,
            )
        self._add_minion_pool_event(context, event_message)
        ...
        if self._allocate_to_action:
            ...
            updated_values["allocation_status"] = constants.MINION_MACHINE_STATUS_IN_USE
        self._update_minion_machine(
            context, self._minion_pool_id, self._minion_machine_id, updated_values
        )
    ...
    def revert(self, context, origin, destination, task_info, **kwargs):
        ...
        with minion_manager_utils.get_minion_pool_lock(
            ...
            machine_db_entry = db_api.get_minion_machine(
            ...
            if machine_db_entry:
                ...
                if not minion_provider_properties and (
                    ...
                    minion_provider_properties = machine_db_entry.provider_properties
                    LOG.debug(
        ...
        if not minion_provider_properties:
            ...
            cleanup_info['minion_provider_properties'] = minion_provider_properties
            try:
                ...
                if not self._raise_on_cleanup_failure:
                    log_msg = "%s Ignoring exception." % log_msg
                log_msg = "%s Exception details were: %s" % (
                    log_msg,
                    utils.get_exception_details,
                )
                LOG.warn(log_msg)
...
class DeallocateMinionMachineTask(BaseMinionManangerTask):
    def __init__(
        ...
        **kwargs,
        ...
        resource_deletion_task_type = constants.TASK_TYPE_DELETE_SOURCE_MINION_MACHINE
        self._raise_on_cleanup_failure = raise_on_cleanup_failure
        if minion_pool_type != constants.PROVIDER_PLATFORM_SOURCE:
    ...
    def execute(self, context, origin, destination, task_info):
        ...
        if machine.provider_properties:
            ...
            execution_info = {"minion_provider_properties": machine.provider_properties}
            try:
            ...
            self._add_minion_pool_event(
                ...
                "removing the machine's entry from the DB" % (self._minion_machine_id),
            )
...
class HealthcheckMinionMachineTask(BaseMinionManangerTask):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        resource_healthcheck_task = constants.TASK_TYPE_HEALTHCHECK_SOURCE_MINION
        if minion_pool_type != constants.PROVIDER_PLATFORM_SOURCE:
        ...
        super(HealthcheckMinionMachineTask, self).__init__(
            minion_pool_id, minion_machine_id, resource_healthcheck_task, **kwargs
        )

    def execute(self, context, origin, destination, task_info):
        res = {"healthy": True, "error": None}
        ...
        if machine.allocation_status in machine_error_statuses:
            base_msg = "Minion Machine with ID '%s' is marked as '%s' in the DB." % (
                self._minion_machine_id,
                machine.allocation_status,
            )
            LOG.debug("[Task '%s'] %s" % (self._task_name, base_msg))
            self._add_minion_pool_event(
                context,
                "%s Reporting healthcheck as failed" % base_msg,
                level=constants.TASK_EVENT_WARNING,
        ...
        try:
            ...
            if not self._fail_on_error:
                res = {"healthy": False, "error": str(ex)}
            else:
                raise
    ...
    def _get_task_name(self, minion_pool_id, minion_machine_id):
        return self.get_healthcheck_task_name(minion_pool_id, minion_machine_id)
...
class MinionMachineHealtchcheckDecider(object):
    ...
    def __init__(
        self, minion_pool_id, minion_machine_id, on_successful_healthcheck=True
    ):
        self._minion_pool_id = minion_pool_id
        self._minion_machine_id = minion_machine_id
    ...
    def __call__(self, history):
        healthcheck_task_name = HealthcheckMinionMachineTask.get_healthcheck_task_name(
        ...
        if healtcheck_result.get('healthy'):
            LOG.debug(
                "Healtcheck task '%s' confirmed worker health. Decider returning %s",
                healthcheck_task_name,
                self._on_success,
            )
...
class PowerOnMinionMachineTask(BaseMinionManangerTask):
    def __init__(
        ...
        **kwargs,
        ...
        self._fail_on_error = fail_on_error
        power_on_task_type = constants.TASK_TYPE_POWER_ON_SOURCE_MINION
        if minion_pool_type != constants.PROVIDER_PLATFORM_SOURCE:
            power_on_task_type = constants.TASK_TYPE_POWER_ON_DESTINATION_MINION
        super(PowerOnMinionMachineTask, self).__init__(
            minion_pool_id, minion_machine_id, power_on_task_type, **kwargs
        )
...
class PowerOffMinionMachineTask(BaseMinionManangerTask):
    def __init__(
        ...
        **kwargs,
        ...
        power_on_task_type = constants.TASK_TYPE_POWER_OFF_SOURCE_MINION
        if minion_pool_type != constants.PROVIDER_PLATFORM_SOURCE:
            power_on_task_type = constants.TASK_TYPE_POWER_OFF_DESTINATION_MINION
