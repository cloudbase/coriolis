...
from oslo_utils import strutils, timeutils
...
class MinionManagerServerEndpoint(object):
    ...
    def _register_refresh_jobs_for_minion_pool(self, minion_pool, period_minutes=None):
        ...
        if period_minutes < 0:
            LOG.warn(
                "Got negative pool refresh period %s. Defaulting to 0.", period_minutes
            )
            period_minutes = 0
        ...
        if period_minutes > 60:
            LOG.warn(
                ...
                "defaulting to 10. Original value was: %s",
                period_minutes,
            )
            period_minutes = 10
        admin_ctxt = context.get_admin_context(minion_pool.maintenance_trust_id)
        description = "Scheduled refresh job for minion pool '%s'" % minion_pool.id

        # NOTE: we need to generate hourly schedules for each minute in
        # the hour we would like the refresh to be triggered:
        for minute in [
            period_minutes * i for i in range(math.ceil(60 / period_minutes))
        ]:
            name = MINION_POOL_REFRESH_CRON_JOB_NAME_FORMAT % (minion_pool.id, minute)
    ...
    def _unregister_refresh_jobs_for_minion_pool(
        ...
        job_prefix = MINION_POOL_REFRESH_JOB_PREFIX_FORMAT % (minion_pool.id)
        try:
    ...
    def _check_minion_pool_capability(self, ctxt, platform_name, provider_type):
        # Raises 405 error if the platform does not support minion pools.
        provider_types = (
            self._rpc_conductor_client.get_available_providers(ctxt)
            .get(platform_name, {})
            .get('types', [])
        )
        if provider_type not in provider_types:
            raise exception.NotSupportedOperation(
                operation="minion pools not supported for platform '%s'" % platform_name
            )
    ...
    def get_endpoint_source_minion_pool_options(
        ...
        worker_service = self._rpc_scheduler_client.get_worker_service_for_specs(
            ...
            provider_requirements={
                endpoint['type']: [constants.PROVIDER_TYPE_SOURCE_MINION_POOL]
            },
        )
    ...
    def get_endpoint_destination_minion_pool_options(
        ...
        worker_service = self._rpc_scheduler_client.get_worker_service_for_specs(
            ...
            region_sets=[[r['id'] for r in endpoint['mapped_regions']]],
            provider_requirements={
                endpoint['type']: [constants.PROVIDER_TYPE_DESTINATION_MINION_POOL]
            },
        )
        worker_rpc = rpc_worker_client.WorkerClient.from_service_definition(
            worker_service
    ...
    def _add_minion_pool_event(self, ctxt, minion_pool_id, level, message):
        LOG.info("Minion pool event for pool %s: %s", minion_pool_id, message)
    ...
    def _add_minion_pool_progress_update(
        ...
        LOG.info("Adding pool progress update for %s: %s", minion_pool_id, message)
    ...
    def _check_keys_for_action_dict(
        ...
        if not isinstance(action, dict):
            raise exception.InvalidInput(
                "Action must be a dict, got '%s': %s" % (type(action), action)
            )
        missing = [prop for prop in required_action_properties if prop not in action]
        if missing:
            raise exception.InvalidInput(
                "Missing the following required action properties for "
                "%s: %s. Got %s" % (operation, missing, action)
            )
    ...
    def validate_minion_pool_selections_for_action(self, ctxt, action):
        ...
        def _check_pool_minion_count(minion_pool, instances, minion_pool_type=""):
            ...
            if desired_minion_count > minion_pool.maximum_minions:
                msg = (
                    ...
                    % (
                        minion_pool.id,
                        minion_pool.maximum_minions,
                        desired_minion_count,
                        action['id'],
                        instances,
                    )
                )
        ...
        if action['origin_minion_pool_id']:
            ...
            if origin_pool.os_type != constants.OS_TYPE_LINUX:
                raise exception.InvalidMinionPoolSelection(
                    ...
                    % (action['origin_minion_pool_id'], origin_pool.os_type)
                )
            _check_pool_minion_count(origin_pool, instances, minion_pool_type="Source")
            LOG.debug(
        ...
        if action['destination_minion_pool_id']:
            ...
            if destination_pool.platform != (constants.PROVIDER_PLATFORM_DESTINATION):
                raise exception.InvalidMinionPoolSelection(
            ...
            if destination_pool.os_type != constants.OS_TYPE_LINUX:
                raise exception.InvalidMinionPoolSelection(
                    ...
                    "transfer minion pool."
                    % (action['destination_minion_pool_id'], destination_pool.os_type)
                )
            _check_pool_minion_count(
                destination_pool, instances, minion_pool_type="Destination"
            )
            LOG.debug(
        ...
        if instance_osmorphing_minion_pool_mappings:
            ...
            for pool_id, instances in osmorphing_pool_mappings.items():
                ...
                _check_pool_minion_count(
                    osmorphing_pool, instances, minion_pool_type="OSMorphing"
                )
                LOG.debug(
    ...
    def allocate_minion_machines_for_transfer(self, ctxt, transfer):
        try:
            ...
            self._cleanup_machines_with_statuses_for_action(
                ctxt, transfer['id'], [constants.MINION_MACHINE_STATUS_UNINITIALIZED]
            )
            self.deallocate_minion_machines_for_action(ctxt, transfer['id'])
            (
                self._rpc_conductor_client.report_transfer_minions_allocation_error(
    ...
    def allocate_minion_machines_for_deployment(
        ...
        try:
            ...
            self._cleanup_machines_with_statuses_for_action(
                ctxt, deployment['id'], [constants.MINION_MACHINE_STATUS_UNINITIALIZED]
            )
            self.deallocate_minion_machines_for_action(ctxt, deployment['id'])
            (
                self._rpc_conductor_client.report_deployment_minions_allocation_error(
    ...
    def _make_minion_machine_allocation_subflow_for_action(
        ...
        extra_available_machine_slots = minion_pool.maximum_minions - len(
            minion_pool.minion_machines
        )
        num_instances = len(action_instances)
        num_currently_available_machines = len(currently_available_machines)
        if num_instances > (
            len(currently_available_machines) + (extra_available_machine_slots)
        ):
            raise exception.InvalidMinionPoolState(
        ...
        def _select_machine(minion_pool, exclude=None):
            ...
            for machine in minion_pool.minion_machines:
                if exclude and machine.id in exclude:
                    LOG.debug(
                        "Excluding minion machine '%s' from search for use action '%s'",
                        machine.id,
                        action_id,
                    )
                    continue
        ...
        return {
            ...
            "action_instance_minion_allocation_mappings": (instance_minion_allocations),
        }
    ...
    def _run_machine_allocation_subflow_for_action(
        ...
        self._check_keys_for_action_dict(
            action, required_action_properties, operation="minion machine selection"
        )
        ...
        if action_type == constants.TRANSFER_ACTION_TYPE_DEPLOYMENT:
            allocation_flow_name_format = (
                minion_mgr_tasks.MINION_POOL_MIGRATION_ALLOCATION_FLOW_NAME_FORMAT
            )
            ...
            machine_action_allocation_subflow_name_format = minion_mgr_tasks.MINION_POOL_ALLOCATE_MACHINES_FOR_MIGRATION_SUBFLOW_NAME_FORMAT  # noqa: E501
        elif action_type == constants.TRANSFER_ACTION_TYPE_TRANSFER:
            allocation_flow_name_format = (
                minion_mgr_tasks.MINION_POOL_REPLICA_ALLOCATION_FLOW_NAME_FORMAT
            )
            allocation_failure_reporting_task_class = (
            ...
            machines_allocation_subflow_name_format = (
                minion_mgr_tasks.MINION_POOL_REPLICA_ALLOCATION_SUBFLOW_NAME_FORMAT
            )
            machine_action_allocation_subflow_name_format = minion_mgr_tasks.MINION_POOL_ALLOCATE_MACHINES_FOR_REPLICA_SUBFLOW_NAME_FORMAT  # noqa: E501
        else:
            raise exception.InvalidInput(
        ...
        # define main flow:
        main_allocation_flow_name = allocation_flow_name_format % action['id']
        ...
        main_allocation_flow.add(allocation_failure_reporting_task_class(action['id']))
        ...
        def _check_pool_allocation_status(minion_pool, minion_pool_type):
            if minion_pool.status != constants.MINION_POOL_STATUS_ALLOCATED:
                raise exception.InvalidMinionPoolState(
                    "Minion Pool '%s' is an invalid state ('%s') to be "
                    "used as a %s pool for action '%s'. The pool must be "
            ...
            with minion_manager_utils.get_minion_pool_lock(
                ...
                allocations_subflow_result = (
                    self._make_minion_machine_allocation_subflow_for_action(
                        ...
                        action['instances'],
                        subflow_name,
                        inject_for_tasks=origin_pool_store,
                    )
                )
                ...
                for (
                    action_instance_id,
                    allocated_minion_id,
                ) in source_machine_allocations.items():
                    instance_machine_allocations[action_instance_id][
                        'origin_minion_id'
                    ] = allocated_minion_id
        ...
        if include_transfer_minions and action['destination_minion_pool_id']:
            ...
            with minion_manager_utils.get_minion_pool_lock(
                ...
                destination_pool_store = self._get_pool_initial_taskflow_store_base(
                    ctxt, minion_pool, endpoint_dict
                )
                ...
                for (
                    action_instance_id,
                    allocated_minion_id,
                ) in destination_machine_allocations.items():
                    instance_machine_allocations[action_instance_id][
        ...
        for action_instance_id, mapped_pool_id in action[
            ...
            if mapped_pool_id not in osmorphing_pool_instance_mappings:
                osmorphing_pool_instance_mappings[mapped_pool_id] = [action_instance_id]
            else:
        ...
        if include_osmorphing_minions and osmorphing_pool_instance_mappings:
            for (
                osmorphing_pool_id,
                action_instance_ids,
            ) in osmorphing_pool_instance_mappings.items():
                ...
                if action['destination_minion_pool_id'] and (
                    ...
                    and (
                        osmorphing_pool_id == (action['destination_minion_pool_id'])
                        and (include_transfer_minions)
                    )
                ):
                    LOG.debug(
                    ...
                    for instance in action_instance_ids:
                        instance_machine_allocations[instance][
                ...
                with minion_manager_utils.get_minion_pool_lock(
                    ...
                    _check_pool_allocation_status(minion_pool, "OSMorphing")
                    endpoint_dict = self._rpc_conductor_client.get_endpoint(
                        ctxt, minion_pool.endpoint_id
                    )
                    ...
                    for (
                        action_instance_id,
                        allocated_minion_id,
                    ) in osmorphing_machine_allocations.items():
                        instance_machine_allocations[action_instance_id][
                            'osmorphing_minion_id'
                        ] = allocated_minion_id
        ...
        LOG.info(
            ...
            % (main_allocation_flow_name, action['id'], instance_machine_allocations)
        )
    ...
    def _cleanup_machines_with_statuses_for_action(
        ...
        if not machines:
            LOG.debug(
                "No minion machines allocated to action '%s'. Returning.", action_id
            )
            return
        ...
        for pool_id, machines in pool_machine_mappings.items():
            with minion_manager_utils.get_minion_pool_lock(pool_id, external=True):
                for machine in machines:
                    LOG.debug(
    ...
    def deallocate_minion_machine(self, ctxt, minion_machine_id):
        ...
        minion_machine = db_api.get_minion_machine(ctxt, minion_machine_id)
        ...
        with minion_manager_utils.get_minion_pool_lock(
            ...
            if (
                minion_machine.allocation_status != machine_allocated_status
            ...
            LOG.debug(
                "Successfully deallocated minion machine with '%s'.", minion_machine.id
            )
    ...
    def _get_healtchcheck_flow_for_minion_machine(
        ...
        machine_healthcheck_subflow = graph_flow.Flow(
            (minion_mgr_tasks.MINION_POOL_HEALTHCHECK_MACHINE_SUBFLOW_NAME_FORMAT)
            % (minion_pool.id, minion_machine.id)
        )

        # add healtcheck task to healthcheck subflow:
        machine_healthcheck_task = minion_mgr_tasks.HealthcheckMinionMachineTask(
            minion_pool.id,
            minion_machine.id,
            minion_pool.platform,
        ...
        machine_reallocation_subflow = linear_flow.Flow(
            (minion_mgr_tasks.MINION_POOL_REALLOCATE_MACHINE_SUBFLOW_NAME_FORMAT)
            % (minion_pool.id, minion_machine.id)
        )
        ...
        machine_healthcheck_subflow.link(
            ...
            decider=minion_mgr_tasks.MinionMachineHealtchcheckDecider(
                minion_pool.id, minion_machine.id, on_successful_healthcheck=False
            ),
        )

        return machine_healthcheck_subflow

    def _get_minion_pool_refresh_flow(self, ctxt, minion_pool, requery=True):
        ...
        pool_refresh_flow = unordered_flow.Flow(
            minion_mgr_tasks.MINION_POOL_REFRESH_FLOW_NAME_FORMAT % (minion_pool.id)
        )
        ...
        for machine in minion_pool.minion_machines:
            if machine.allocation_status not in (healthcheckable_machine_statuses):
            ...
            if machine.last_used_at:
                expiry_time = machine.last_used_at + datetime.timedelta(
            ...
            if max_minions_to_deallocate > 0 and minion_expired:
                if minion_pool.minion_retention_strategy == (
                    ...
                    raise exception.InvalidMinionPoolState(
                        ...
                        "'%s'" % (minion_pool.minion_retention_strategy, minion_pool.id)
                    )
                max_minions_to_deallocate = max_minions_to_deallocate - 1
        ...
        if skipped_machines:
            ...
            LOG.debug("[Pool '%s'] %s: %s", minion_pool.id, base_msg, skipped_machines)
            self._add_minion_pool_event(
                ctxt,
                minion_pool.id,
                constants.TASK_EVENT_INFO,
        ...
        if machines_to_healthcheck:
            self._add_minion_pool_event(
                ...
                "of the refreshing of the minion pool: %s" % (machines_to_healthcheck),
            )
            for machine in machines_to_healthcheck:
                db_api.set_minion_machine_allocation_status(
                    ctxt, machine, constants.MINION_MACHINE_STATUS_HEALTHCHECKING
                )
        else:
            self._add_minion_pool_event(
                ctxt,
                minion_pool.id,
                constants.TASK_EVENT_INFO,
                "No minion machines require healthchecking during pool refresh",
            )
    ...
    def refresh_minion_pool(self, ctxt, minion_pool_id):
        ...
        acceptable_allocation_statuses = [constants.MINION_POOL_STATUS_ALLOCATED]
        ...
        if current_status not in acceptable_allocation_statuses:
            raise exception.InvalidMinionPoolState(
                ...
                % (minion_pool_id, current_status, acceptable_allocation_statuses)
            )
        ...
        self._taskflow_runner.run_flow_in_background(refresh_flow, store=initial_store)
        self._add_minion_pool_event(
            ctxt,
            minion_pool.id,
            constants.TASK_EVENT_INFO,
    ...
    def _get_minion_pool_allocation_flow(self, minion_pool):
        ...
        allocation_flow = linear_flow.Flow(
            minion_mgr_tasks.MINION_POOL_ALLOCATION_FLOW_NAME_FORMAT % (minion_pool.id)
        )

        # tansition pool to VALIDATING:
        allocation_flow.add(
            minion_mgr_tasks.UpdateMinionPoolStatusTask(
                minion_pool.id,
                constants.MINION_POOL_STATUS_VALIDATING_INPUTS,
        ...
        allocation_flow.add(
            minion_mgr_tasks.ValidateMinionPoolOptionsTask(
        ...
        allocation_flow.add(
            minion_mgr_tasks.UpdateMinionPoolStatusTask(
                minion_pool.id, constants.MINION_POOL_STATUS_ALLOCATING_SHARED_RESOURCES
            )
        )
        ...
        fmt = minion_mgr_tasks.MINION_POOL_ALLOCATE_MINIONS_SUBFLOW_NAME_FORMAT
        ...
        if machines_flow:
            allocation_flow.add(
                minion_mgr_tasks.UpdateMinionPoolStatusTask(
                    minion_pool.id, constants.MINION_POOL_STATUS_ALLOCATING_MACHINES
                )
            )
            LOG.debug(
    ...
    def create_minion_pool(
        ...
        endpoint_dict = self._rpc_conductor_client.get_endpoint(ctxt, endpoint_id)
    ...
    def _get_pool_initial_taskflow_store_base(self, ctxt, minion_pool, endpoint_dict):
    ...
    def _check_pool_machines_in_use(
        ...
        used_machines = {
            mch
            for mch in minion_pool.minion_machines
        ...
        if used_machines and raise_if_in_use:
            ...
            raise exception.InvalidMinionPoolState(
                ...
                " active state: %s" % (minion_pool.id, mch_id)
            )
    ...
    def allocate_minion_pool(self, ctxt, minion_pool_id):
        ...
        acceptable_allocation_statuses = [constants.MINION_POOL_STATUS_DEALLOCATED]
        current_status = minion_pool.status
        if current_status not in acceptable_allocation_statuses:
            raise exception.InvalidMinionPoolState(
                ...
                % (minion_pool_id, minion_pool.status, acceptable_allocation_statuses)
            )
        ...
        try:
            db_api.set_minion_pool_status(
                ctxt, minion_pool_id, constants.MINION_POOL_STATUS_POOL_MAINTENANCE
            )
            self._taskflow_runner.run_flow_in_background(
                allocation_flow, store=initial_store
            )
            ...
            db_api.set_minion_pool_status(ctxt, minion_pool_id, current_status)
            raise
    ...
    def _get_minion_pool_deallocation_flow(self, minion_pool, raise_on_error=True):
        """Returns a taskflow.Flow object pertaining to all the tasks
        ...
        if machines_flow:
            ...
            LOG.debug("No machines for pool '%s' require deallocating.", minion_pool.id)
    ...
    def _get_pool_deallocation_initial_store(self, ctxt, minion_pool, endpoint_dict):
        base = self._get_pool_initial_taskflow_store_base(
            ctxt, minion_pool, endpoint_dict
        )
        if 'task_info' not in base:
            base['task_info'] = {}
        base['task_info']['pool_shared_resources'] = minion_pool.shared_resources
        return base
    ...
    def deallocate_minion_pool(self, ctxt, minion_pool_id, force=False):
        ...
        if current_status == constants.MINION_POOL_STATUS_DEALLOCATED:
            LOG.debug(
                ...
                "Nothing to do so returning early.",
                minion_pool_id,
            )
            return self._get_minion_pool(
                ctxt,
        ...
        self._check_pool_machines_in_use(ctxt, minion_pool, raise_if_in_use=not force)
        endpoint_dict = self._rpc_conductor_client.get_endpoint(
            ctxt, minion_pool.endpoint_id
        )
    ...
    def delete_minion_pool(self, ctxt, minion_pool_id):
        minion_pool = self._get_minion_pool(ctxt, minion_pool_id, include_machines=True)
        ...
        if minion_pool.status not in acceptable_deletion_statuses:
            raise exception.InvalidMinionPoolState(
                ...
                % (minion_pool_id, minion_pool.status, acceptable_deletion_statuses)
            )
        self._unregister_refresh_jobs_for_minion_pool(minion_pool, raise_on_error=False)
