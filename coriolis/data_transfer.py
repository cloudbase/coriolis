        ...
        if os.path.exists(CONF.compressor_address):
            ...
            if stat.S_ISSOCK(mode):
                return (
                    ...
                    "http+unix://%s/" % parse.quote_plus(CONF.compressor_address),
                )
            else:
...
def compression_proxy(content, fmt):
    ...
    if None in (sess, url):
        ...
        try:
            ...
            ret = sess.post(
                url, data=data, headers=headers, timeout=CONF.default_requests_timeout
            )
            ret.raise_for_status()
            compressed_data = ret.content
        except Exception as err:
            LOG.exception("failed to compress using coriolis-compressor: %s" % err)
    ...
    LOG.debug("Compression space saving: {:.02f}%".format(compression_saving))
...
def encode_data(msg_id, path, offset, content, compress=True):
    inflated_content = path.encode() + b'\0' + struct.pack("<Q", offset) + content
