...
_CORIOLIS_DISK_SYNC_RESOURCES_INFO_SCHEMA_NAME = "disk_sync_resources_info_schema.json"
...
def get_schema(package_name, schema_name, schemas_directory=DEFAULT_SCHEMAS_DIRECTORY):
    ...
    try:
        ...
        package_loader = jinja2.PackageLoader(parent_package, schemas_directory)
    ...
    LOG.debug(
        "Succesfully loaded and parsed schema '%s' from '%s'.",
        schema_name,
        package_name,
    )
    return schema
...
def validate_value(val, schema, format_checker=None, raise_on_error=True):
    ...
    try:
        ...
        if raise_on_error:
            ...
            LOG.warn(
                "Schema validation failed, ignoring: %s", utils.get_exception_details()
            )
        return False
...
SCHEDULE_API_BODY_SCHEMA = get_schema(__name__, _SCHEDULE_API_BODY_SCHEMA_NAME)
...
CORIOLIS_NETWORK_MAP_SCHEMA = get_schema(__name__, _CORIOLIS_NETWORK_MAP_SCHEMA_NAME)

CORIOLIS_STORAGE_MAPPINGS_SCHEMA = get_schema(
    __name__, _CORIOLIS_STORAGE_MAPPINGS_SCHEMA_NAME
)

CORIOLIS_VM_STORAGE_SCHEMA = get_schema(__name__, _CORIOLIS_VM_STORAGE_SCHEMA_NAME)
