...
TASK_RETURN_VALUE_FORMAT = "%s-result" % (constants.TASK_LOCK_NAME_FORMAT)
LOG = logging.getLogger(__name__)

taskflow_opts = [
    cfg.IntOpt(
        "worker_task_execution_timeout",
        default=3600,
...
class BaseCoriolisTaskflowTask(taskflow_tasks.Task):
    ...
    def _get_error_str_for_flow_failures(self, flow_failures, full_tracebacks=True):
        ...
        for task_id, task_failure in flow_failures.items():
            ...
            if full_tracebacks:
                ...
                failure_str = task_failure.exception_str
                if isinstance(task_failure.exception, exception.TaskProcessException):
...
class BaseRunWorkerTask(BaseCoriolisTaskflowTask):
    ...
    def _set_requires_for_dependencies(self, kwargs, depends_on):
        dep_requirements = [TASK_RETURN_VALUE_FORMAT % dep_id for dep_id in depends_on]
    ...
    def _set_requires_for_task_info_fields(self, kwargs):
        ...
        if self._cleanup_task_runner_type:
            ...
            cleanup_task_deps = list(
                set(cleanup_task_runner.get_required_task_info_properties()).difference(
    ...
    def _set_provides_for_task_info_fields(self, kwargs):
        ...
        if self._cleanup_task_runner_type:
            ...
            cleanup_task_res = list(
                set(cleanup_task_runner.get_returned_task_info_properties()).difference(
                    main_task_runner.get_returned_task_info_properties()
                )
            )
    ...
    def _get_worker_service_rpc_for_task(
        ...
        task_info = {"id": task_id, "task_type": task_type}
    ...
    def _execute_task(self, ctxt, task_id, task_type, origin, destination, task_info):
        ...
        try:
            LOG.debug(
                ...
                "on worker service." % (self._task_id, self._task_name, task_type)
            )
            ...
            LOG.debug(
                ...
                "(type '%s') on the worker service: %s",
                self._task_name,
                task_id,
                task_type,
    ...
    def revert(self, context, origin, destination, task_info, **kwargs):
        ...
        LOG.debug(
            ...
            % (self._task_name, self._task_id, self._cleanup_task_runner_type, res)
        )
