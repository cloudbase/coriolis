...
from coriolis import constants, events, exception, schemas, utils
...
def _check_ensure_volumes_info_ordering(export_info, volumes_info):
    ...
    instance = export_info.get(
        'instance_name', export_info.get('name', export_info['id'])
    )

    vol_info_cpy = utils.sanitize_task_info({"volumes_info": volumes_info}).get(
        "volumes_info", []
    )
    ...
    for disk in export_info['devices']['disks']:
        disk_id = disk['id']
        matching_volumes = [vol for vol in volumes_info if vol['disk_id'] == disk_id]
        if not matching_volumes:
            LOG.error(
                "Could not find source disk '%s' (ID '%s') in Replica volumes info: %s",
                disk,
                disk_id,
                vol_info_cpy,
            )
            ...
            LOG.error(
                "Multiple disks with ID '%s' found in Replica volumes info: %s",
                disk_id,
                vol_info_cpy,
            )
            raise exception.InvalidActionTasksExecutionState(
    ...
    ordered_vol_info_cpy = utils.sanitize_task_info(
        {"volumes_info": ordered_volumes_info}
    ).get("volumes_info", [])

    LOG.debug(
        "volumes_info returned by provider for instance '%s': %s",
        instance,
        vol_info_cpy,
    )
    LOG.debug(
        "volumes_info for instance '%s' after reordering: %s",
        instance,
        ordered_vol_info_cpy,
    )
...
def _update_legacy_disk_ids_in_volumes_info(export_info, volumes_info):
    ...
    existing_volume_disk_ids = {str(v.get('disk_id')) for v in volumes_info}
    for volume_info in volumes_info:
        ...
        if new_disk_id in existing_volume_disk_ids:
            ...
            continue
        LOG.info(
            "Migrating volumes_info entry from legacy disk_id '%s' to '%s'.",
            disk_id,
            new_disk_id,
        )
...
class ShutdownInstanceTask(base.TaskRunner):
    ...
    def _run(self, ctxt, instance, origin, destination, task_info, event_handler):
        ...
        source_environment = task_info['source_environment']
        provider.shutdown_instance(ctxt, connection_info, source_environment, instance)
        return {}
...
class ReplicateDisksTask(base.TaskRunner):
    ...
    def _run(self, ctxt, instance, origin, destination, task_info, event_handler):
        ...
        if migr_source_conn_info:
            ...
            migr_source_conn_info = base.unmarshal_migr_conn_info(migr_source_conn_info)
        ...
        if migr_target_conn_info:
            ...
            migr_target_conn_info['connection_details'] = base.unmarshal_migr_conn_info(
        ...
        schemas.validate_value(volumes_info, schemas.CORIOLIS_VOLUMES_INFO_SCHEMA)

        volumes_info = _check_ensure_volumes_info_ordering(export_info, volumes_info)
...
class DeleteReplicaSourceDiskSnapshotsTask(base.TaskRunner):
    ...
    def _run(self, ctxt, instance, origin, destination, task_info, event_handler):
        ...
        if not task_info.get("volumes_info"):
            LOG.debug("No volumes_info present. Skipping source snapshot deletion.")
...
class DeleteReplicaSourceResourcesTask(base.TaskRunner):
    ...
    def _run(self, ctxt, instance, origin, destination, task_info, event_handler):
        ...
        return {"source_resources": None, "source_resources_connection_info": None}
...
class DeployReplicaTargetResourcesTask(base.TaskRunner):
    ...
    @classmethod
    def get_returned_task_info_properties(cls):
        return ["volumes_info", "target_resources", "target_resources_connection_info"]
    ...
    def _validate_connection_info(self, migr_connection_info):
        try:
            backup_writers.BackupWritersFactory(migr_connection_info, None).get_writer()
        except Exception as err:
            LOG.warn(
                "Seemingly invalid backup writer conn info. Replica will "
                "likely fail during disk Replication. Error is: %s" % (str(err))
            )
...
class DeleteReplicaTargetResourcesTask(base.TaskRunner):
    ...
    @classmethod
    def get_returned_task_info_properties(cls):
        return ["target_resources", "target_resources_connection_info"]
    ...
    def _run(self, ctxt, instance, origin, destination, task_info, event_handler):
        ...
        return {"target_resources": None, "target_resources_connection_info": None}
...
class DeployReplicaInstanceResourcesTask(base.TaskRunner):
    ...
    @classmethod
    def get_required_task_info_properties(cls):
        return ["export_info", "target_environment", "clone_disks", "volumes_info"]
    ...
    def _run(self, ctxt, instance, origin, destination, task_info, event_handler):
        ...
        return {"instance_deployment_info": import_info['instance_deployment_info']}
...
class ValidateReplicaExecutionDestinationInputsTask(base.TaskRunner):
    ...
    def _run(self, ctxt, instance, origin, destination, task_info, event_handler):
        ...
        destination_connection_info = base.get_connection_info(ctxt, destination)
        ...
        if any(d.get("owner") and d.get("owner") != instance for d in disks if d):
    ...
    def _run(self, ctxt, instance, origin, destination, task_info, event_handler):
        ...
        if not destination_provider:
            event_manager.progress_update(
                ...
                "support Replica Deployment input validation" % (destination_type)
            )
            return {}
...
class UpdateSourceReplicaTask(base.TaskRunner):
    ...
    def _run(self, ctxt, instance, origin, destination, task_info, event_handler):
        ...
        if not new_source_env:
            event_manager.progress_update("No new source environment options provided")
            return {'volumes_info': volumes_info, 'source_environment': old_source_env}
...
class UpdateDestinationReplicaTask(base.TaskRunner):
    ...
    def _run(self, ctxt, instance, origin, destination, task_info, event_handler):
        ...
        volumes_info = destination_provider.check_update_destination_environment_params(
            ctxt,
            destination_connection_info,
            export_info,
            volumes_info,
