...
class TaskRunner(with_metaclass(abc.ABCMeta)):
    def get_shared_libs_for_providers(self, ctxt, origin, destination, event_handler):
        ...
        if platform in [
            ...
            required_libs.extend(
                get_shared_lib_dirs_for_provider(ctxt, destination, event_handler)
            )
    ...
    def get_required_provider_types(cls):
        ...
        """
        raise NotImplementedError(
            "No required provider types specified for task class of type '%s'." % cls
        )
    ...
    def get_required_platform(cls):
        ...
        """
        raise NotImplementedError(
            "No required platform specified for task class of type '%s'." % cls
        )
    ...
    def run(self, ctxt, instance, origin, destination, task_info, event_handler):
        ...
        missing_info_props = [
            prop
            for prop in self.get_required_task_info_properties()
            if prop not in task_info
        ]
        if missing_info_props:
            raise exception.CoriolisException(
                "Task type '%s' asked to run on task info with "
                "missing properties: %s" % (self.__class__, missing_info_props)
            )
        ...
        if type(result) is not dict:
            raise exception.CoriolisException(
                ...
                "instead of a dict: %s" % (self.__class__, type(result), result)
            )

        missing_returns = [
            prop
            for prop in self.get_returned_task_info_properties()
        ...
        if missing_returns:
            raise exception.CoriolisException(
                ...
                "Result was: %s"
                % (self.__class__, missing_returns, utils.sanitize_task_info(result))
            )
        ...
        if undeclared_returns:
            raise exception.CoriolisException(
                ...
                "keys in its result: %s" % (self.__class__, undeclared_returns)
            )
...
def marshal_migr_conn_info(migr_connection_info, private_key_field_name="pkey"):
    if migr_connection_info and (private_key_field_name in migr_connection_info):
        ...
        if isinstance(pkey, str) is False:
            migr_connection_info[private_key_field_name] = utils.serialize_key(
                pkey, CONF.serialization.temp_keypair_password
            )
    return migr_connection_info


def unmarshal_migr_conn_info(migr_connection_info, private_key_field_name="pkey"):
    if migr_connection_info and (private_key_field_name in migr_connection_info):
        migr_connection_info = migr_connection_info.copy()
        pkey_str = migr_connection_info[private_key_field_name]
        if isinstance(pkey_str, paramiko.rsakey.RSAKey) is False:
            migr_connection_info[private_key_field_name] = utils.deserialize_key(
