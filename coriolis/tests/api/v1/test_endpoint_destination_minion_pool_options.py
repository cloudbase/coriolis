...
from coriolis.api.v1 import endpoint_destination_minion_pool_options as endpoint
...
class EndpointDestinationMinionPoolOptionsControllerTestCase(
    ...
    def setUp(self):
        super(EndpointDestinationMinionPoolOptionsControllerTestCase, self).setUp()
        self.minion_api = endpoint.EndpointDestinationMinionPoolOptionsController()

    @mock.patch.object(utils, 'decode_base64_param')
    @mock.patch.object(
        endpoint_options_view, 'destination_minion_pool_options_collection'
    )
    @mock.patch.object(api.API, 'get_endpoint_destination_minion_pool_options')
    def test_index(
        self,
        ...
        (mock_get_endpoint_destination_minion_pool_options.assert_called_once_with)(
            mock_context, endpoint_id, env=env, option_names=options
        )
        (mock_destination_minion_pool_options_collection.assert_called_once_with)(
            mock_get_endpoint_destination_minion_pool_options.return_value
        )
        self.assertEqual(
            mock_destination_minion_pool_options_collection.return_value, result
        )
