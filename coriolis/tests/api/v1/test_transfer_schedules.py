...
from coriolis import exception, schemas
...
class TransferScheduleControllerTestCase(test_base.CoriolisBaseTestCase):
    ...
    def setUp(self):
        ...
        self.transfer_schedules = transfer_schedules.TransferScheduleController()

    @mock.patch.object(transfer_schedule_view, 'single')
    @mock.patch.object(api.API, 'get_schedule')
    def test_show(self, mock_get_schedule, mock_single):
        mock_req = mock.Mock()
        ...
        mock_context.can.assert_called_once_with("migration:transfer_schedules:show")
        mock_get_schedule.assert_called_once_with(mock_context, transfer_id, id)
        mock_single.assert_called_once_with(mock_get_schedule.return_value)

    @mock.patch.object(transfer_schedule_view, 'single')
    @mock.patch.object(api.API, 'get_schedule')
    def test_show_not_found(self, mock_get_schedule, mock_single):
        mock_req = mock.Mock()
        mock_context = mock.Mock()
        mock_req.environ = {'coriolis.context': mock_context}
        id = mock.sentinel.id
        ...
        self.assertRaises(
            exc.HTTPNotFound, self.transfer_schedules.show, mock_req, transfer_id, id
        )
    ...
    def test_index(self, mock_get_schedules, mock_collection):
        mock_req = mock.Mock()
        mock_context = mock.Mock()
        mock_req.environ = {'coriolis.context': mock_context}
        transfer_id = mock.sentinel.transfer_id
        ...
        mock_context.can.assert_called_once_with("migration:transfer_schedules:list")
        mock_get_schedules.assert_called_once_with(
            mock_context, transfer_id, expired=False
        )
        mock_collection.assert_called_once_with(mock_get_schedules.return_value)
    ...
    def test_validate_expiration_date_is_none(self):
        expiration_date = None

        result = self.transfer_schedules._validate_expiration_date(expiration_date)

        self.assertEqual(None, result)

    def test_validate_expiration_date_past(self):
    ...
    def test_validate_expiration_date(self):
        ...
        self.assertEqual(datetime.datetime(9999, 12, 31), result)

    @mock.patch.object(
        transfer_schedules.TransferScheduleController, '_validate_expiration_date'
    )
    @mock.patch.object(schemas, 'validate_value')
    @mock.patch.object(jsonschema, 'FormatChecker')
    @mock.patch.object(
        transfer_schedules.TransferScheduleController, '_validate_schedule'
    )
    def test_validate_create_body(
        ...
        mock_validate_value.assert_called_once_with(
            mock_body,
            schemas.SCHEDULE_API_BODY_SCHEMA,
    ...
    def test_validate_update_body(
        ...
        result = self.transfer_schedules._validate_update_body(mock_update_body)
    ...
    @mock.patch.object(
        transfer_schedules.TransferScheduleController, '_validate_create_body'
    )
    def test_create(self, mock_validate_create_body, mock_create, mock_single):
        ...
        mock_context.can.assert_called_once_with("migration:transfer_schedules:create")
    ...
    @mock.patch.object(
        transfer_schedules.TransferScheduleController, '_validate_update_body'
    )
    def test_update(self, mock_validate_update_body, mock_update, mock_single):
        ...
        result = self.transfer_schedules.update(mock_req, transfer_id, id, body)

        self.assertEqual(mock_single.return_value, result)

        mock_context.can.assert_called_once_with("migration:transfer_schedules:update")
        mock_validate_update_body.assert_called_once_with(body)
        mock_update.assert_called_once_with(
            mock_context, transfer_id, id, mock_validate_update_body.return_value
        )
    ...
    def test_delete(self, mock_delete):
        ...
        self.assertRaises(
            exc.HTTPNoContent, self.transfer_schedules.delete, mock_req, transfer_id, id
        )

        mock_context.can.assert_called_once_with("migration:transfer_schedules:delete")
