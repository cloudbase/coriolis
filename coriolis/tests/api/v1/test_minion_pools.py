...
class MinionPoolControllerTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test_show(self, mock_get_minion_pool, mock_single):
        ...
        mock_single.assert_called_once_with(mock_get_minion_pool.return_value)
    ...
    def test_show_not_found(self, mock_get_minion_pool, mock_single):
        ...
        self.assertRaises(exc.HTTPNotFound, self.minion_pools.show, mock_req, id)
    ...
    def test_index(self, mock_get_minion_pools, mock_collection):
        mock_req = mock.Mock()
        mock_context = mock.Mock()
        mock_req.environ = {'coriolis.context': mock_context}
        ...
        mock_collection.assert_called_once_with(mock_get_minion_pools.return_value)
    ...
    def test__check_pool_retention_strategy(
        ...
        if exception_raised:
            self.assertRaisesRegex(
                Exception,
                "Invalid minion pool retention strategy '%s'" % pool_retention_strategy,
            ...
            self.assertEqual(
                None, self.minion_pools._check_pool_retention_strategy(strategy)
            )

    @ddt.file_data('data/minion_pools_numeric_values.yml')
    def test__check_pool_numeric_values(self, config, exception_raised):
    ...
    @mock.patch.object(
        minion_pools.MinionPoolController, '_check_pool_retention_strategy'
    )
    @mock.patch.object(minion_pools.MinionPoolController, '_check_pool_numeric_values')
    ...
    def test__validate_create_body(
        ...
        minion_pool["os_type"] = getattr(constants, minion_pool["os_type"], None)
        minion_pool["platform"] = getattr(constants, minion_pool["platform"], None)
        if 'minion_retention_strategy' in minion_pool:
            minion_pool["minion_retention_strategy"] = getattr(
                constants,
        ...
        if exception_raised:
            self.assertRaisesRegex(
                Exception,
                exception_raised,
                testutils.get_wrapped_function(self.minion_pools._validate_create_body),
                self.minion_pools,
                ctxt,
                body,
            )
            ...
            with mock.patch.object(
                endpoints_api.API, expected_validation_api_method
            ) as mock_validation_api_method:
                ...
                mock_validation_api_method.assert_called_once_with(
                    ctxt, minion_pool["endpoint_id"], minion_pool["environment_options"]
                )
    ...
    @mock.patch.object(minion_pools.MinionPoolController, '_validate_create_body')
    def test_create(self, mock_validate_create_body, mock_create, mock_single):
        ...
        mock_context.can.assert_called_once_with("migration:minion_pools:create")
    ...
    def test__validate_updated_environment_options(
        self, platform, expected_api_method, exception_raised
    ):
        platform = getattr(constants, platform, None)
        endpoint_id = mock.sentinel.endpoint_id
        minion_pool = {'platform': platform, 'endpoint_id': endpoint_id}
        ...
        if exception_raised:
            ...
            with mock.patch.object(
                endpoints_api.API, expected_api_method
            ) as mock_api_method:
                self.minion_pools._validate_updated_environment_options(
                    context, minion_pool, enviroment_options
                )
    ...
    @mock.patch.object(
        minion_pools.MinionPoolController, '_validate_updated_environment_options'
    )
    @mock.patch.object(api.API, 'get_minion_pool')
    @mock.patch.object(
        minion_pools.MinionPoolController, '_check_pool_retention_strategy'
    ...
    def test__validate_update_body(
        ...
        minion_retention_strategy = minion_pool.get('minion_retention_strategy', "")
        ...
        mock_context = mock.Mock()
        id = mock.sentinel.id
        mock_get_minion_pool.return_value = {'minimum_minions': 1, 'maximum_minions': 1}

        if exception_raised:
            self.assertRaisesRegex(
                Exception,
                exception_raised,
                testutils.get_wrapped_function(self.minion_pools._validate_update_body),
                self.minion_pools,
                id,
                mock_context,
                body,
            ...
            if environment_options:
                (mock_validate_updated_environment_options.assert_called_once_with)(
                    mock_context, mock_get_minion_pool.return_value, environment_options
                )

    @mock.patch.object(minion_pool_view, 'single')
    @mock.patch.object(api.API, 'update')
    @mock.patch.object(minion_pools.MinionPoolController, '_validate_update_body')
    def test_update(self, mock_validate_update_body, mock_update, mock_single):
        ...
        mock_context.can.assert_called_once_with("migration:minion_pools:update")
    ...
    def test_delete(self, mock_delete):
        ...
        self.assertRaises(exc.HTTPNoContent, self.minion_pools.delete, mock_req, id)
    ...
    def test_delete_not_found(self, mock_delete):
        ...
        self.assertRaises(exc.HTTPNotFound, self.minion_pools.delete, mock_req, id)

        mock_context.can.assert_called_once_with("migration:minion_pools:delete")
        mock_delete.assert_called_once_with(mock_context, id)
