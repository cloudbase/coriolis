...
class ServiceControllerTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test_show(
        ...
        mock_get_service.assert_called_once_with(mock_context, id)
    ...
    def test_show_no_service(
        ...
        self.assertRaises(exc.HTTPNotFound, self.services.show, mock_req, id)
    ...
    def test_index(self, mock_get_services, mock_collection):
        mock_req = mock.Mock()
        mock_context = mock.Mock()
        mock_req.environ = {'coriolis.context': mock_context}
    ...
    def test_validate_create_body(
        ...
        result = testutils.get_wrapped_function(self.services._validate_create_body)(
            self.services, body=body
        )

        self.assertEqual((host, binary, topic, mapped_regions, enabled), result)
    ...
    def test_validate_update_body(self):
        host = mock.sentinel.host
        binary = mock.sentinel.binary
        topic = mock.sentinel.topic
        mapped_regions = ["region_1", "region_2"]
        ...
        result = testutils.get_wrapped_function(self.services._validate_update_body)(
            self.services, body=body
        )

        self.assertEqual({'enabled': enabled, 'mapped_regions': mapped_regions}, result)

    def test_validate_update_body_no_keys(self):
        service = {}
        body = {"service": service}
    ...
    def test_delete(self, mock_delete):
        ...
        self.assertRaises(exc.HTTPNoContent, self.services.delete, mock_req, id)
    ...
    def test_delete_not_found(self, mock_delete):
        ...
        self.assertRaises(exc.HTTPNotFound, self.services.delete, mock_req, id)
