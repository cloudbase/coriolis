...
class EndpointActionsControllerTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test_validate_connection(self, mock_validate_connection):
        mock_req = mock.Mock()
        ...
        result = testutils.get_wrapped_function(self.endpoint_api._validate_connection)(
            mock_req,
            id,
            body,  # type: ignore
        )
    ...
    @mock.patch.object(api.API, 'validate_connection')
    def test_validate_connection_except_not_found(self, mock_validate_connection):
        mock_req = mock.Mock()
        mock_context = mock.Mock()
        mock_req.environ = {'coriolis.context': mock_context}
        id = mock.sentinel.id
        ...
        self.assertRaises(
            exc.HTTPNotFound,
            testutils.get_wrapped_function(self.endpoint_api._validate_connection),
            mock_req,
            id,
            body,
        )
