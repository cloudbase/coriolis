...
@ddt.ddt
class TransferTasksExecutionActionsControllerTestCase(test_base.CoriolisBaseTestCase):
    ...
    def setUp(self):
        ...
        self.transfer_api = transfer_api.TransferTasksExecutionActionsController()

    @mock.patch.object(api.API, 'cancel')
    @ddt.file_data('data/transfer_task_execution_actions_cancel.yml')
    def test_cancel(
        self, mock_cancel, config, expected_force, exception_raised, expected_result
    ):
        mock_req = mock.Mock()
        ...
        body = config["body"]
        if exception_raised:
            mock_cancel.side_effect = getattr(exception, exception_raised)("err")
        ...
        mock_context.can.assert_called_once_with("migration:transfer_executions:cancel")
