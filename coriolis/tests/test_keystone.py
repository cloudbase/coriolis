...
from coriolis import exception, keystone
...
class KeystoneTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test_create_trust(
        ...
        mock_get_plugin_loader.return_value.load_from_options.return_value = self.auth
        ...
        mock_get_plugin_loader.return_value.load_from_options.assert_called_once_with(
    ...
    @mock.patch.object(keystone.loading, 'get_plugin_loader')
    @mock.patch.object(keystone.ks_session, 'Session')
    def test_create_trust_unauthorized_exception(
        self,
        mock_session,
        mock_get_plugin_loader,
        mock_client,
        ...
        mock_session.return_value = self.session
        mock_client.return_value = self.client
        self.trusts_auth_plugin.get_user_id.side_effect = ks_exceptions.Unauthorized

        self.assertRaises(exception.NotAuthorized, keystone.create_trust, self.ctxt)

    @mock.patch('coriolis.keystone._get_trusts_auth_plugin')
    @mock.patch.object(keystone.kc_v3, 'Client')
    @mock.patch.object(keystone.ks_session, 'Session')
    def test_delete_trust(self, mock_session, mock_client, mock_get_trusts_auth_plugin):
        mock_get_trusts_auth_plugin.return_value = self.trusts_auth_plugin
        self.ctxt.trust_id = 'test_trust_id'
        mock_session.return_value = self.session
        mock_client.return_value = self.client
        ...
        mock_session.assert_called_once_with(auth=self.trusts_auth_plugin, verify=True)
        mock_client.assert_called_once_with(session=self.session)
    ...
    @mock.patch.object(keystone.ks_session, 'Session')
    @mock.patch.object(keystone.loading, 'get_plugin_loader')
    def test_create_keystone_session(self, mock_get_plugin_loader, mock_session):
        mock_get_plugin_loader.return_value.load_from_options.return_value = self.auth
        mock_session.return_value = self.session

        result = keystone.create_keystone_session(self.ctxt, self.connection_info)
    ...
    def test_create_keystone_session_with_trust_id_no_username(
        self, mock_get_plugin_loader, mock_session, mock_get_trusts_auth_plugin
    ):
        mock_get_plugin_loader.return_value.load_from_options.return_value = self.auth
        mock_session.return_value = self.session
        mock_get_trusts_auth_plugin.return_value = self.trusts_auth_plugin
    ...
    def test_create_keystone_session_with_connection_info_no_username(
        ...
        result = keystone.create_keystone_session(self.ctxt, connection_info)
    ...
    @mock.patch.object(keystone.ks_session, 'Session')
    @mock.patch.object(keystone.loading, 'get_plugin_loader')
    def test_create_keystone_session_version_3(
        self, mock_get_plugin_loader, mock_session
    ):
        mock_get_plugin_loader.return_value.load_from_options.return_value = self.auth
        mock_session.return_value = self.session
