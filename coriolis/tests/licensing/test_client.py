...
class LicensingClientTestCase(test_base.CoriolisBaseTestCase):
    ...
    def setUp(self):
        ...
        self.client = licensing_module.LicensingClient("https://10.7.2.3:37667/v1")
    ...
    def test_from_env(
        self, mock_get_licence_status, mock_create_appliance, mock_get_appliances
    ):
    ...
    def test_from_env_multiple_appliances(self, mock_get_appliances):
        ...
        self.assertRaises(
            exception.CoriolisException, licensing_module.LicensingClient.from_env
        )

    @mock.patch.object(licensing_module.LicensingClient, 'get_appliances')
    @mock.patch.object(licensing_module.LicensingClient, 'create_appliance')
    @mock.patch.object(licensing_module.LicensingClient, 'get_licence_status')
    def test_from_env_no_appliances(
        self, mock_get_licence_status, mock_create_appliance, mock_get_appliances
    ):
        os.environ["LICENSING_SERVER_BASE_URL"] = "https://10.7.2.3:37667/v1"
        mock_get_appliances.return_value = []
        ...
        mock_get_licence_status.assert_called_once()
        self.assertEqual(result._appliance_id, mock_create_appliance.return_value["id"])
    ...
    def test__get_url_for_appliance_resource(self):
        ...
        self.assertEqual(
            result, "https://10.7.2.3:37667/v1/appliances/appliance_id/licences"
        )
    ...
    def test_raise_response_error_conflict(self):
        ...
        self.assertRaises(
            exception.Conflict, self.client._raise_response_error, mock_response
        )
    ...
    @mock.patch.object(licensing_module.LicensingClient, '_raise_response_error')
    @mock.patch('requests.post')
    def test__do_req(self, mock_post, mock_raise_response_error):
    ...
    def test__do_req_invalid_method(self):
        ...
        self.assertRaises(
            ValueError, original_do_req, self.client, 'INVALID_METHOD', self.resource
        )

    @mock.patch.object(licensing_module.LicensingClient, '_raise_response_error')
    @mock.patch('requests.get')
    def test__do_req_raw_response(self, mock_get, mock_raise_response_error):
        mock_response = self.setup_mock_response(mock_get)

        original_do_req = testutils.get_wrapped_function(self.client._do_req)

        result = original_do_req(self.client, 'GET', self.resource, raw_response=True)
    ...
    @mock.patch.object(licensing_module.LicensingClient, '_raise_response_error')
    @mock.patch('requests.get')
    def test__do_req_response_not_ok(self, mock_get, mock_raise_response_error):
        mock_response = self.setup_mock_response(
            mock_get,
            ok=False,
            status_code=409,
            json_return_value={'error': {'code': 409, 'message': 'test_message'}},
        )
        ...
        self.assertEqual(result, {'error': {'code': 409, 'message': 'test_message'}})
        mock_raise_response_error.assert_called_once_with(mock_response)

    @mock.patch.object(licensing_module.LicensingClient, '_raise_response_error')
    @mock.patch('requests.get')
    def test__do_req_no_response_key(self, mock_get, mock_raise_response_error):
        mock_response = self.setup_mock_response(
    ...
    def test_create_appliance(self, mock_post):
        ...
        mock_post.assert_called_once_with(
            '/appliances', body=None, response_key='appliance', appliance_scoped=False
        )
    ...
    def test_get_licence_status(self, mock_get):
        ...
        mock_get.assert_called_once_with('/status', 'appliance_licence_status')
    ...
    def test_get_licences(self, mock_get):
        ...
        mock_get.assert_called_once_with('/licences', response_key='licences')
    ...
    def test_add_licence(self, mock_post):
        ...
        mock_post.assert_called_once_with('/licences', licence_data)
    ...
    def test_add_reservation(self, mock_post):
        ...
        mock_post.assert_called_once_with(
            '/reservations',
            {'type': licensing_module.RESERVATION_TYPE_REPLICA, 'count': 2},
    ...
    def test_add_reservation_sap_types(self, mock_post):
        for sap_type in [
            licensing_module.RESERVATION_TYPE_SAP_MIGRATION,
    ...
    def test_add_reservation_invalid_type(self):
        self.assertRaises(
            ValueError,
            self.client.add_reservation,
            'invalid_reservation_type',
            mock.sentinel.num_vms,
        )
    ...
    def test_get_reservations(self, mock_get):
        ...
        mock_get.assert_called_once_with('/reservations', response_key='reservations')
    ...
    def test_delete_reservation_404_status_code(
        self, mock_do_req, mock_raise_response_error
    ):
        self.setup_mock_response(mock_do_req, ok=False, status_code=404)

        with self.assertLogs('coriolis.licensing.client', level=logging.WARN):
            self.client.delete_reservation(self.reservation_id, raise_on_404=False)

        mock_do_req.assert_called_once_with(
            'delete', '/reservations/%s' % self.reservation_id, raw_response=True
        )
        mock_raise_response_error.assert_not_called()
    ...
    def test_delete_reservation_404_status_code_raise(
        ...
        mock_response = self.setup_mock_response(mock_do_req, ok=False, status_code=404)
    ...
    def test_delete_reservation_status_code_raise(
        ...
        mock_response = self.setup_mock_response(mock_do_req, ok=False, status_code=400)
    ...
    @mock.patch.object(licensing_module.LicensingClient, '_raise_response_error')
    @mock.patch.object(licensing_module.LicensingClient, '_do_req')
    def test_delete_reservation(self, mock_do_req, mock_raise_response_error):
        mock_response = self.setup_mock_response(mock_do_req, ok=False, status_code=200)
