...
class MinionManagerServerEndpointTestCase(test_base.CoriolisBaseTestCase):
    ...
    @ddt.data(
        {
            "available_types": [constants.PROVIDER_TYPE_DESTINATION_MINION_POOL],
    ...
    @ddt.unpack
    def test__check_minion_pool_capability(self, available_types, expect_exception):
        available_providers = {}
        if available_types is not None:
            available_providers = {"foo": {"types": available_types}}

        mock_conductor_client = mock.MagicMock()
        mock_conductor_client.get_available_providers.return_value = available_providers
