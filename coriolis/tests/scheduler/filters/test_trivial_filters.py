...
class RegionsFilterTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test__repr__(self):
        ...
        self.assertEqual(
            result,
            "<RegionsFilter(regions=%s, "
            "any_region=%s)>" % (self.regions, self.any_region),
        )
...
class ProviderTypesFilterTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test__repr__(self):
        ...
        self.assertEqual(
            result,
            "<ProviderTypesFilter(provider_requirements=%s)>" % self.provider_types,
        )

    @ddt.data(
        ({'platform1': ['type1']}, {'platform1': {'types': ['type1']}}, 100),
        ({'platform1': ['type1']}, {'platform2': {'types': ['type1']}}, 0),
        ({'platform1': ['type1', 'type2']}, {'platform1': {'types': ['type1']}}, 0),
    )
    @ddt.unpack
    def test_rate_service(self, provider_requirements, providers, expected):
        self.provider_types_filter._provider_requirements = provider_requirements
