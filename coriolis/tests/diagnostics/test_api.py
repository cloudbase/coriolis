...
class DiagnosticsAPITestCase(test_base.CoriolisBaseTestCase):
    ...
    def test_get(self, mock_get_diagnostics_info):
        (self.diagnostics_api._conductor_cli.get_all_diagnostics.return_value) = [
            mock.sentinel.all_diag
        ]
        mock_get_diagnostics_info.return_value = mock.sentinel.diag_info
        expected_result = [mock.sentinel.all_diag, mock.sentinel.diag_info]

        result = self.diagnostics_api.get(mock.sentinel.context)

        self.assertEqual(expected_result, result)
        (
            self.diagnostics_api._conductor_cli.get_all_diagnostics.assert_called_once_with
