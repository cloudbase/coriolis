...
class DatabaseSqlalchemyApiTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test_db_sync_version(self, mock_db_version, mock_db_sync):
        ...
        mock_db_sync.assert_called_once_with(mock.sentinel.engine, version=1)

    @mock.patch.object(api, 'db_version')
    def test_db_sync_version_raise(self, mock_db_version):
        mock_db_version.return_value = 2

        self.assertRaises(
            exception.CoriolisException, api.db_sync, mock.sentinel.engine, version=1
        )
        mock_db_version.assert_called_once_with(mock.sentinel.engine)
