...
class ServiceTestCase(test_base.CoriolisBaseTestCase):
    ...
    @ddt.data(
        ...
        ([], None, [], False),
        (['--worker-process-count', '5', '--unknown-arg'], 5, ['--unknown-arg'], False),
    ...
    @ddt.unpack
    def test_get_worker_count_from_args(
        self, input_args, expected_count, expected_unknown_args, expect_exception
    ):
        if expect_exception:
            self.assertRaises(
                SystemExit, service.get_worker_count_from_args, input_args
            )
        else:
            worker_count, unknown_args = service.get_worker_count_from_args(input_args)
    ...
    @ddt.data(
        ...
        ({'lock_path': "/path/to/locks"}, True, True, ['file1', 'file2'], logging.WARN),
    ...
    def test_check_locks_dir_empty(
        self,
        oslo_concurrency,
        exists,
        isdir,
        ...
        with mock.patch.object(service.CONF, 'oslo_concurrency', oslo_concurrency):
...
class WSGIServiceTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test_init(self, mock_server, mock_conf, mock_rpc, mock_load_app):
        ...
        wsgi_service = service.WSGIService(
            mock.sentinel.name, worker_count=10, init_rpc=True
        )
        ...
        self.assertEqual(wsgi_service._port, mock_conf.api_migration_listen_port)
    ...
    def test_init_windows(
        self, mock_conf, mock_get_transport, mock_load_app, mock_server, mock_system
    ):
    ...
    @mock.patch('oslo_service.wsgi.Loader.load_app')
    @mock.patch('coriolis.rpc.messaging.get_transport')
    def test_service_methods(self, mock_get_transport, mock_load_app, mock_conf):
        ...
        self.assertEqual(result.get_workers_count(), mock_conf.api_migration_workers)
...
class MessagingServiceTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test_init(self, mock_get_hostname, mock_target, mock_conf, mock_rpc):
        ...
        self.assertEqual(messaging_service._server, mock_rpc.get_server.return_value)
    ...
    @mock.patch.object(service.utils, 'get_hostname')
    def test_init_windows(
        self, mock_get_hostname, mock_target, mock_conf, mock_rpc, mock_system
    ):
        mock_system.return_value = "Windows"
        mock_conf.api_migration_listen = mock.sentinel.listen
        mock_conf.api_migration_listen_port = mock.sentinel.listen_port
    ...
    def test_init_with_none_worker_count(
        self, mock_get_worker_count, mock_get_hostname, mock_conf, mock_rpc
    ):
        mock_conf.api_migration_listen = mock.sentinel.listen
        mock_conf.api_migration_listen_port = mock.sentinel.listen_port
        mock_conf.api_migration_listen = mock.sentinel.listen
    ...
    def test_service_methods(
        ...
        self.assertEqual(result.get_workers_count(), mock.sentinel.worker_count)
