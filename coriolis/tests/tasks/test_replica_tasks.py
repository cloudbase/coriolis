    ...
    def test__get_volumes_info(self, data):
        ...
        if data[1]:
            self.assertRaises(
                ...
                replica_tasks._get_volumes_info,
                task_info,
            )
        else:
...
class GetInstanceInfoTaskTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test__run(self, mock_validate_value, mock_get_conn_info, mock_get_provider):
...
class ReplicateDisksTaskTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test__run(
        ...
        task_info.get.side_effect = [
            task_info['incremental'],
            task_info['source_resources'],
        ]
        ...
        expected_unmarshal_calls = [
            ...
            mock.call.mock_unmarshal(
                task_info['target_resources_connection_info']['connection_details']
            ),
        ]
...
class DeployReplicaDisksTaskTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test__run(
        ...
        mock_get_conn_info.assert_called_once_with(mock.sentinel.ctxt, destination)
        prov_fun.assert_called_once_with(
            mock.sentinel.ctxt,
            mock_get_conn_info.return_value,
...
class DeleteReplicaSourceDiskSnapshotsTaskTestCase(test_base.CoriolisBaseTestCase):
    def setUp(self):
    ...
    @mock.patch('coriolis.tasks.base.get_connection_info')
    @mock.patch.object(replica_tasks, '_get_volumes_info')
    def test__run(self, mock_get_vol_info, mock_get_conn_info, mock_get_provider, _):
        origin = mock.MagicMock()
        task_info = mock.MagicMock()
        task_info.get.side_effect = [task_info['volumes_info']]
        prov_fun = mock_get_provider.return_value.delete_replica_source_snapshots
...
class DeleteReplicaDisksTaskTestCase(test_base.CoriolisBaseTestCase):
    ...
    @mock.patch('coriolis.tasks.base.get_connection_info')
    @mock.patch.object(replica_tasks, '_get_volumes_info')
    def test__run(self, mock_get_vol_info, mock_get_conn_info, mock_get_provider):
        destination = mock.MagicMock()
        task_info = mock.MagicMock()
        task_info.get.side_effect = [task_info['volumes_info']]
...
class DeployReplicaSourceResourcesTaskTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test__run(
        ...
        prov_fun = mock_get_provider.return_value.deploy_replica_source_resources
        prov_fun.return_value = replica_resources_info
        mock_marshal.return_value = replica_resources_info.get('connection_info', {})
        ...
        def _get_result():
            ...
            if log_expected:
                with self.assertLogs(
                    'coriolis.tasks.replica_tasks', level=replica_tasks.logging.WARN
                ):
...
class DeleteReplicaSourceResourcesTaskTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test__run(self, mock_get_conn_info, mock_get_provider):
        origin = mock.MagicMock()
        task_info = mock.MagicMock()
        prov_fun = mock_get_provider.return_value.delete_replica_source_resources
...
class DeployReplicaTargetResourcesTaskTestCase(test_base.CoriolisBaseTestCase):
    ...
    @ddt.unpack
    def test__validate_connection_info(
        self,
        mock_validate,
        mock_marshal,
        mock_backup_writer,
    ...
    @mock.patch.object(
        replica_tasks.DeployReplicaTargetResourcesTask, '_validate_connection_info'
    )
    @mock.patch('coriolis.providers.factory.get_provider')
    @mock.patch('coriolis.tasks.base.get_connection_info')
    @mock.patch('coriolis.tasks.replica_tasks._get_volumes_info')
    @mock.patch('coriolis.tasks.replica_tasks._check_ensure_volumes_info_ordering')
    @mock.patch('coriolis.schemas.validate_value')
    ...
    def test__run(
        ...
        mock_get_conn_info,
        mock_get_provider,
        mock_validate_conn_info,
        replica_resources_info,
        expected_result,
    ):
        destination = mock.MagicMock()
        task_info = mock.MagicMock()
        prov_fun = mock_get_provider.return_value.deploy_replica_target_resources
        prov_fun.return_value = replica_resources_info
        mock_check_vol_info.return_value = replica_resources_info.get('volumes_info')
...
class DeleteReplicaTargetResourcesTaskTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test__run(self, data, mock_get_conn_info, mock_get_provider):
        ...
        prov_fun = mock_get_provider.return_value.delete_replica_target_resources
        expected_result = {
...
class DeployReplicaInstanceResourcesTaskTestCase(test_base.CoriolisBaseTestCase):
    def setUp(self):
...
class FinalizeReplicaInstanceDeploymentTaskTestCase(test_base.CoriolisBaseTestCase):
    def setUp(self):
        super(FinalizeReplicaInstanceDeploymentTaskTestCase, self).setUp()
        self.task_runner = replica_tasks.FinalizeReplicaInstanceDeploymentTask()
    ...
    def test__run(self, mock_get_conn_info, mock_get_provider):
        destination = mock.MagicMock()
        task_info = mock.MagicMock()
        prov_fun = mock_get_provider.return_value.finalize_replica_instance_deployment
...
class CleanupFailedReplicaInstanceDeploymentTaskTestCase(
    ...
    def setUp(self):
        ...
        self.task_runner = replica_tasks.CleanupFailedReplicaInstanceDeploymentTask()
    ...
    def test__run(self, mock_get_conn_info, mock_get_provider):
        ...
        task_info = mock.MagicMock()
        prov_fun = (
            mock_get_provider.return_value.cleanup_failed_replica_instance_deployment
        )
...
class CreateReplicaDiskSnapshotsTaskTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test__run(
        self,
        mock_check_ensure_volumes_ordering,
        mock_validate_value,
        mock_get_vol_info,
        mock_get_conn_info,
        mock_get_provider,
...
class DeleteReplicaTargetDiskSnapshotsTaskTestCase(test_base.CoriolisBaseTestCase):
    def setUp(self):
    ...
    def test__run(
        ...
        destination = mock.MagicMock()
        task_info = mock.MagicMock()
        prov_fun = mock_get_provider.return_value.delete_replica_target_disk_snapshots
        expected_result = {
            "volumes_info": mock_check_ensure_volumes_ordering.return_value
        }
...
class RestoreReplicaDiskSnapshotsTaskTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test__run(
        ...
        destination = mock.MagicMock()
        task_info = mock.MagicMock()
        prov_fun = mock_get_provider.return_value.restore_replica_disk_snapshots
        expected_result = {
            "volumes_info": mock_check_ensure_volumes_ordering.return_value
        }
...
class ValidateReplicaExecutionSourceInputsTaskTestCase(test_base.CoriolisBaseTestCase):
    def setUp(self):
    ...
    @mock.patch('coriolis.providers.factory.get_provider')
    @mock.patch('coriolis.tasks.base.get_connection_info')
    def test__run(self, mock_get_conn_info, mock_get_provider, mock_event_manager):
        origin = mock.MagicMock()
        task_info = mock.MagicMock()
    ...
    def test__run_no_source_provider(
        self, mock_get_conn_info, mock_get_provider, mock_event_manager
    ):
        origin = mock.MagicMock()
        task_info = mock.MagicMock()
        prov_fun = mock_get_provider.return_value.validate_replica_export_input
...
class ValidateReplicaExecutionDestinationInputsTaskTestCase(
    ...
    def setUp(self):
        super(ValidateReplicaExecutionDestinationInputsTaskTestCase, self).setUp()
        self.task_runner = replica_tasks.ValidateReplicaExecutionDestinationInputsTask()
    ...
    def test__run(
        self, mock_validate_replica_inputs, mock_get_conn_info, mock_get_provider
    ):
        destination = mock.MagicMock()
        task_info = mock.MagicMock()
...
class ValidateReplicaDeploymentParametersTaskTestCase(test_base.CoriolisBaseTestCase):
    def setUp(self):
        super(ValidateReplicaDeploymentParametersTaskTestCase, self).setUp()
        self.task_runner = replica_tasks.ValidateReplicaDeploymentParametersTask()
    ...
    def test__run(
        ...
        destination = mock.MagicMock()
        task_info = mock.MagicMock()
        prov_fun = mock_get_provider.return_value.validate_replica_deployment_input
...
class UpdateSourceReplicaTaskTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test__run_no_new_source_env(
        ...
        prov_fun = mock_get_provider.return_value.check_update_source_environment_params
        expected_result = {
    ...
    def test__run_no_volumes_info(
        ...
        new_source_env = {"opt": "new"}
        task_info = mock.MagicMock()
        task_info.get.side_effect = [volumes_info, new_source_env, volumes_info]
        prov_fun = mock_get_provider.return_value.check_update_source_environment_params
        prov_fun.return_value = None
        expected_result = {
            "volumes_info": volumes_info,
    ...
    def test__run(
        ...
        prov_fun = mock_get_provider.return_value.check_update_source_environment_params
        expected_result = {
            "volumes_info": prov_fun.return_value,
            "source_environment": new_source_env,
        }
        result = self.task_runner._run(
            mock.sentinel.ctxt,
...
class UpdateDestinationReplicaTaskTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test__run_no_new_dest_env(
        ...
        task_info.get.side_effect = [volumes_info, None]
        prov_fun = (
            mock_get_provider.return_value.check_update_destination_environment_params
        )
        expected_result = {
            "volumes_info": volumes_info,
            "target_environment": old_dest_env,
        }
