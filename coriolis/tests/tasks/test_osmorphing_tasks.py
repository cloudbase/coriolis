...
class OSMorphingTaskTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test__run(
        ...
        mock_unmarshal.assert_called_once_with(task_info['osmorphing_connection_info'])
...
class DeployOSMorphingResourcesTaskTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test__run(
        ...
        def _get_result(*args):
            if log_expected:
                with self.assertLogs(
                    'coriolis.tasks.osmorphing_tasks',
        ...
        if raise_expected:
            self.assertRaises(
                exception.InvalidTaskResult, self.task_runner._run, *method_args
            )
