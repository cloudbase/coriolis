...
class GetOptimalFlavorTaskTestCase(test_base.CoriolisBaseTestCase):
    ...
    @ddt.data(None, {"selected_flavor": "flavor1"})
    def test__run(
        self,
...
class ValidateMigrationDestinationInputsTaskTestCase(test_base.CoriolisBaseTestCase):
    def setUp(self):
        super(ValidateMigrationDestinationInputsTaskTestCase, self).setUp()
        self.task_runner = migration_tasks.ValidateMigrationDestinationInputsTask()
    ...
    def test__validate_provider_replica_import_input(self):
        ...
        self.task_runner._validate_provider_replica_import_input(provider, *args)
        provider.validate_replica_import_input.assert_called_once_with(
            *args, check_os_morphing_resources=True, check_final_vm_params=True
        )
