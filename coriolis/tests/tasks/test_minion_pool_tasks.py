...
MOCK_MINION_BACKUP_WRITER_CONN_INFO = {"connection_details": {"user": "username"}}
...
class CoriolisBaseMinionPoolTaskTestCase(test_base.CoriolisBaseTestCase):
    ...
    def _setup_base_mocks(self):
        ...
        get_provider_patcher = mock.patch('coriolis.providers.factory.get_provider')
        ...
        mp_tasks_methods_patches = mock.patch.multiple(
            ...
            _get_minion_conn_info=mock.MagicMock(return_value=MOCK_MINION_CONN_INFO),
            _get_minion_backup_writer_conn_info=mock.MagicMock(
                return_value=MOCK_MINION_BACKUP_WRITER_CONN_INFO
            ),
            _get_platform_to_target=mock.MagicMock(return_value=self.destination),
        )
        mp_tasks_methods_patches.start()
        self.addCleanup(mp_tasks_methods_patches.stop)

        get_conn_info_patcher = mock.patch('coriolis.tasks.base.get_connection_info')
    ...
    def _setup_vol_attachment_tasks_mocks(self):
        self.provider_disk_op = mock.MagicMock(
            return_value=(self.mock_get_provider.return_value.attach_volumes_to_minion)
        )
    ...
    def _setup_validation_tasks_mocks(self):
        prov_fun = self.mock_get_provider.return_value.validate_minion_compatibility_for_transfer
        self.provider_pool_validation_op = mock.MagicMock(return_value=prov_fun)
        ...
        patcher = mock.patch.multiple(
            ...
            _get_provider_pool_validation_operation=(self.provider_pool_validation_op),
        )
        patcher.start()
        self.addCleanup(patcher.stop)
...
class MinionPoolTasksMethodsTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test__get_platform_to_target(self, data):
        ...
        if not data[0]:
            self.assertRaises(data[1], mp_tasks._get_platform_to_target, *args)
            return
        ...
        def _get_result():
            if logs_missing_props:
                with self.assertLogs(
                    'coriolis.tasks.minion_pool_tasks', mp_tasks.logging.WARN
                ):
            ...
            return mp_tasks._check_missing_minion_properties(
                mock.ANY, minion_properties, properties_to_check=properties_to_check
            )
    ...
    @ddt.data(
        ...
        (
            ...
            {"connection_details": MOCK_MARSHALED_INFO, "backup": "writer"},
            True,
        ),
    )
    def test__get_minion_backup_writer_conn_info(self, data):
        minion_properties = data[0]
        connection_details = (
            minion_properties.get('backup_writer_connection_info', {})
        ...
        result = mp_tasks._get_minion_backup_writer_conn_info(minion_properties)
        self.assertEqual(result, expected_result)
        if marshaled:
            self.mock_marshal_conn_info.assert_called_once_with(connection_details)


class _BaseValidateMinionPoolOptsTaskTestCase(CoriolisBaseMinionPoolTaskTestCase):
    def setUp(self):
    ...
    def test__run(self):
        ...
        self.mock_get_provider.assert_called_once_with(
            self.destination['type'], self.provider_type, mock.sentinel.event_handler
        )
        self.mock_get_provider.return_value.validate_minion_pool_environment_options.assert_called_once_with(
            mock.sentinel.ctxt,
            self.mock_get_conn_info.return_value,
...
class _BaseCreateMinionMachineTaskTestCase(CoriolisBaseMinionPoolTaskTestCase):
    ...
    def test__run(self):
        ...
        expected_result = {
            ...
            "minion_provider_properties": (prov_fun.return_value.get.return_value),
        }
...
class _BaseSetUpPoolSupportingResourcesTaskTestCase(CoriolisBaseMinionPoolTaskTestCase):
    def setUp(self):
    ...
    def test__run(self):
        prov_fun = self.mock_get_provider.return_value.set_up_pool_shared_resources
        expected_result = {"pool_shared_resources": prov_fun.return_value}
...
class _BaseTearDownPoolSupportingResourcesTaskTestCase(
    ...
    def test__run(self):
        prov_fun = self.mock_get_provider.return_value.tear_down_pool_shared_resources
        expected_result = {"pool_shared_resources": None}
...
@ddt.ddt
class _BaseVolumesMinionMachineAttachmentTask(CoriolisBaseMinionPoolTaskTestCase):
    def setUp(self):
    ...
    def test_get_required_task_info_properties(self):
        ...
        result = mp_tasks._BaseVolumesMinionMachineAttachmentTask.get_required_task_info_properties()
    ...
    def test_get_returned_task_info_properties(self):
        ...
        result = mp_tasks._BaseVolumesMinionMachineAttachmentTask.get_returned_task_info_properties()
        self.assertEqual(sorted(result), sorted(expected_result))
    ...
    def test__check_missing_disk_op_result_keys(self, data):
        ...
        if raise_expected:
            self.assertRaises(
                ...
                mock.sentinel.platform_to_target,
                disk_op_res,
            )
        else:
            self.assertEqual(
    ...
    def test__run(self, mock_check_missing_disk_op_keys):
        ...
        expected_result = {
            ...
            MOCK_MINION_PROPS_TASK_INFO_FIELD: prov_fun_result['minion_properties'],
            "target_resources": (prov_fun_result['minion_properties']),
...
class _BaseAttachVolumesToTransferMinionTaskTestCase(test_base.CoriolisBaseTestCase):
    def test_get_required_task_info_properties(self):
        ...
        with mock.patch.object(
            ...
            result = mp_tasks._BaseAttachVolumesToTransferMinionTask.get_required_task_info_properties()
            self.assertEqual(
    ...
    def test_get_returned_task_info_properties(self):
        ...
        with mock.patch.object(
            ...
            result = mp_tasks._BaseAttachVolumesToTransferMinionTask.get_returned_task_info_properties()
            self.assertEqual(
                sorted(result), sorted(['field1', 'field2', 'volumes_info'])
            )
...
@ddt.ddt
class AttachVolumesToOSMorphingMinionTaskTestCase(test_base.CoriolisBaseTestCase):
    def setUp(self):
    ...
    def test_get_required_task_info_properties(self):
        ...
        with mock.patch.object(
            ...
            result = mp_tasks.AttachVolumesToOSMorphingMinionTask.get_required_task_info_properties()
            self.assertEqual(
                sorted(result), sorted(['field1', 'field2', 'instance_deployment_info'])
            )
    ...
    def test_get_returned_task_info_properties(self):
        ...
        with mock.patch.object(
            ...
            result = mp_tasks.AttachVolumesToOSMorphingMinionTask.get_returned_task_info_properties()
            self.assertEqual(
                sorted(result), sorted(['field1', 'field2', 'instance_deployment_info'])
            )

    @mock.patch.object(mp_tasks._BaseVolumesMinionMachineAttachmentTask, '_run')
    @ddt.data(
        {
...
@ddt.ddt
class _BaseValidateMinionCompatibilityTaskTestCase(CoriolisBaseMinionPoolTaskTestCase):
    def setUp(self):
    ...
    @mock.patch.object(
        mp_tasks._BaseValidateMinionCompatibilityTask,
    ...
    def test_get_required_task_info_properties(self, _):
        ...
        result = mp_tasks._BaseValidateMinionCompatibilityTask.get_required_task_info_properties()
        self.assertEqual(sorted(result), sorted(expected_result))

    def test_get_returned_task_info_properties(self):
        expected_result = ["target_resources", "target_resources_connection_info"]
    ...
    def test__get_transfer_properties_task_info_field(self, data):
        test_fun = mp_tasks._BaseValidateMinionCompatibilityTask._get_transfer_properties_task_info_field
        with mock.patch.object(
            mp_tasks._BaseValidateMinionCompatibilityTask,
            'get_required_platform',
            return_value=data[0],
        ):
            if not data[1]:
...
class _BaseReleaseMinionTaskTestCase(test_base.CoriolisBaseTestCase):
    ...
    @mock.patch.object(
        mp_tasks._BaseReleaseMinionTask, 'get_returned_task_info_properties'
    )
...
class CollectOSMorphingInfoTaskTestCase(CoriolisBaseMinionPoolTaskTestCase):
    ...
    def test__run(self, data):
        prov_fun = self.mock_get_provider.return_value.get_additional_os_morphing_info
        ...
        if raises_exception:
            self.assertRaises(exception.CoriolisException, self.task_runner._run, *args)
            return
...
class _BaseHealthcheckMinionMachineTaskTestCase(CoriolisBaseMinionPoolTaskTestCase):
    def setUp(self):
    ...
    @mock.patch(
        'coriolis.tasks.base.unmarshal_migr_conn_info',
...
class _BasePowerCycleMinionTaskTestCase(CoriolisBaseMinionPoolTaskTestCase):
    ...
    @mock.patch.object(mp_tasks._BasePowerCycleMinionTask, '_get_minion_power_cycle_op')
