...
class BaseBackupWriterTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test_open_with_exception(self, mock_get_impl):
        mock_get_impl.return_value._open.side_effect = CoriolisTestException()
        ...
        impl._handle_exception.assert_called_once_with(mock.ANY)
        impl.close.assert_called_once()
...
class FileBackupWriterTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test_from_connection_info(self):
        result = self.writer.from_connection_info(
            mock.sentinel.info, mock.sentinel.volume_info)
...
class SSHBackupWriterImplTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test__send_msg(self):
        ...
        self.writer._send_msg(mock.sentinel.data)
    ...
    def test__send_msg_with_exception(self):
        ...
        self.assertEqual(self._stdout.channel.exit_status_ready.call_count, 5)
        self.assertEqual(self._stdout.channel.recv_exit_status.call_count, 5)
    ...
    def test__sender(self, mock_send_msg):
        self._sender_queue.get.side_effect = [
            mock.sentinel.data, CoriolisTestException()]
        self.writer._sender_q = self._sender_queue
        self.writer._sender_q.task_done.return_value = None

        self.assertRaises(CoriolisTestException, self.writer._sender)

        self.assertEqual(self._sender_queue.get.call_count, 2)
        mock_send_msg.assert_called_once_with(mock.sentinel.data)
        self._sender_queue.task_done.assert_called_once()

    @mock.patch.object(backup_writers.SSHBackupWriterImpl, '_send_msg')
    def test__sender_with_exception(self, mock_send_msg):
        self.writer._sender_q = self._sender_queue
        mock_send_msg.side_effect = exception.CoriolisException()

        self.assertRaises(exception.CoriolisException, self.writer._sender)

        self.assertEqual(self._sender_queue.get.call_count, 1)
        self._sender_queue.task_done.assert_called_once()
    ...
    def test__encoder(self, mock_encode_data):
        ...
        self._enc_queue.get.side_effect = [
        ...
        self._enc_queue.get.assert_called()
        mock_encode_data.assert_called_once_with(
            mock.sentinel.data, mock.sentinel.offset, mock.sentinel.msg_id)
        self._sender_queue.put.assert_called_once_with(
            mock_encode_data.return_value)
        self._enc_queue.task_done.assert_called_once()

    @mock.patch.object(backup_writers.SSHBackupWriterImpl, '_encode_data')
    def test__encoder_exception(self, mock_encode_data):
        self._enc_queue.get.return_value = {
            "data": mock.sentinel.data,
            "offset": mock.sentinel.offset,
            "msg_id": mock.sentinel.msg_id}

        mock_encode_data.side_effect = exception.CoriolisException()
        self.writer._enc_q = self._enc_queue

        self.assertRaises(exception.CoriolisException, self.writer._encoder)

        self._enc_queue.get.assert_called_once()
        self._enc_queue.task_done.assert_called_once()
...
class SSHBackupWriterTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test_from_connection_info_missing_required_field(self):
        ...
        self.assertRaises(exception.CoriolisException,
                          self.writer.from_connection_info,
    ...
    def test__copy_helper_cmd(self, mock_ssh_client):
        ...
        self.writer._copy_helper_cmd(mock_ssh)
    ...
    def test__copy_helper_cmd_stat_error(self, mock_ssh_client):
        ...
        self.assertRaises(IOError, self.writer._copy_helper_cmd, mock_ssh)

        self._sftp.put.assert_not_called()
        self.assertEqual(self._sftp.close.call_count, 5)

    @mock.patch('paramiko.SSHClient')
    def test__connect_ssh(self, mock_ssh_client):
        mock_ssh_client.return_value = self._ssh

        result = self.writer._connect_ssh()
    ...
    def test__connect_ssh_with_exception(self, mock_sleep, mock_ssh_client):
        mock_ssh_client.return_value = self._ssh
        self._ssh.connect.side_effect = CoriolisTestException()

        self.assertRaises(CoriolisTestException, self.writer._connect_ssh)

        self.assertEqual(self._ssh.close.call_count, 5)
...
class HTTPBackupWriterImplTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test__acquire(self, mock_session_class, mock_conf, mock_uri,
        ...
        self.writer._uri = mock_uri.return_value
        self.writer._session = mock_session

        self.writer._acquire()

        mock_ensure_session.assert_called_once()
        mock_session.get.assert_called_once_with(
            f"{self.writer._uri}/acquire",
    ...
    def test__release(self, mock_session_class, mock_conf, mock_uri,
        ...
        self.writer._release()

        mock_ensure_session.assert_called_once()
        mock_session.get.assert_called_once_with(
            f"{self.writer._uri}/release",
            headers={"X-Client-Token": self.writer._id},
            timeout=mock_conf.default_requests_timeout)
        mock_response.raise_for_status.assert_called_once()
    ...
    def test__compressor(self, mock_compression_proxy):
        ...
        self.writer._comp_q.get.side_effect = [
            {"offset": mock.sentinel.offset,
             "data": mock.sentinel.data},
            CoriolisTestException()]
        mock_compression_proxy.return_value = (mock.sentinel.data, "gzip")
        ...
        self.writer._comp_q.task_done.assert_called_once()

    @mock.patch('coriolis.data_transfer.compression_proxy')
    def test__compressor_with_exception(self, mock_compression_proxy):
        self.writer._comp_q = mock.MagicMock()
        self.writer._sender_q = mock.MagicMock()
        self.writer._compress_transfer = True
        ...
        self.writer._comp_q.get.return_value = {
            "offset": mock.sentinel.offset,
            "data": mock.sentinel.data}
        mock_compression_proxy.side_effect = exception.CoriolisException()

        self.assertRaises(exception.CoriolisException, self.writer._compressor)

        self.writer._comp_q.get.assert_called_once()
        mock_compression_proxy.assert_called_once_with(
            mock.sentinel.data,
            backup_writers.constants.COMPRESSION_FORMAT_GZIP)
    ...
    @mock.patch.object(backup_writers.HTTPBackupWriterImpl, '_uri')
    @mock.patch.object(backup_writers, 'CONF')
    @mock.patch.object(backup_writers, 'copy')
    @mock.patch('requests.Session')
    def test__sender(self, mock_session_class, mock_copy, mock_conf, mock_uri,
                     mock_ensure_session):
        self.writer._uri = mock_uri.return_value
        self.writer._sender_q = mock.MagicMock()
        self.writer._sender_q.get.side_effect = [
            {"offset": mock.sentinel.offset,
             "data": mock.sentinel.data,
             "chunk": mock.sentinel.compressed_data,
             "encoding": "gzip"},
            {"offset": mock.sentinel.offset,
             "data": mock.sentinel.data,
             "chunk": mock.sentinel.compressed_data},
        ...
        expected_headers_1 = {
            "X-Write-Offset": str(mock_copy.copy.return_value),
            "X-Client-Token": mock_copy.copy.return_value,
            "content-encoding": mock_copy.copy.return_value,
        }
        ...
        self.writer._session.post.assert_any_call(
            mock_uri.return_value,
            headers=expected_headers_1,
            data=mock_copy.copy.return_value,
            timeout=mock_conf.default_requests_timeout)
        self.writer._session.post.assert_any_call(
            mock_uri.return_value,
            headers=expected_headers_2,
            data=mock_copy.copy.return_value,
            timeout=mock_conf.default_requests_timeout)
    ...
    @mock.patch.object(backup_writers.HTTPBackupWriterImpl, '_uri')
    @mock.patch('requests.Session')
    def test__sender_exception(self, mock_session_class, mock_uri,
                               mock_ensure_session):
        self.writer._uri = mock_uri.return_value
        mock_session = mock_session_class.return_value
        mock_response = mock.MagicMock()
        ...
        mock_ensure_session.assert_called()
        mock_response.raise_for_status.not_called()
...
class HTTPBackupWriterBootstrapperTestcase(test_base.CoriolisBaseTestCase):
    ...
    def setUp(self):
        ...
        self.writer_port = mock.sentinel.writer_port
        self.ssh_conn_info = {
            "ip": mock.sentinel.ip,
            "port": mock.sentinel.port,
            "pkey": None,
    ...
    def test__connect_ssh(self, mock_ssh_client):
        ...
        result = writer._connect_ssh()

        self._ssh.set_missing_host_key_policy.assert_called_with(
            mock.ANY)
        self._ssh.connect.assert_called_with(
            hostname=writer._ip,
            port=writer._port,
            username=writer._username,
            pkey=writer._pkey,
            password=writer._password
        )
        self.assertEqual(result, self._ssh)
    ...
    @mock.patch('coriolis.utils.exec_ssh_cmd')
    @mock.patch('paramiko.SSHClient')
    def test__inject_dport_allow_rule(self, mock_ssh_client,
                                      mock_exec_ssh_cmd):
        writer = backup_writers.HTTPBackupWriterBootstrapper(
            self.ssh_conn_info, self.writer_port)
        ...
        writer._inject_dport_allow_rule(mock_ssh)
    ...
    @mock.patch('coriolis.utils.exec_ssh_cmd')
    @mock.patch('paramiko.SSHClient')
    def test__inject_dport_allow_rule_with_exception(self, mock_ssh_client,
                                                     mock_exec_ssh_cmd):
        writer = backup_writers.HTTPBackupWriterBootstrapper(
            self.ssh_conn_info, self.writer_port)
    ...
    @mock.patch('coriolis.utils.exec_ssh_cmd')
    @mock.patch('paramiko.SSHClient')
    def test__add_firewalld_port(self, mock_ssh_client, mock_exec_ssh_cmd):
        writer = backup_writers.HTTPBackupWriterBootstrapper(
            self.ssh_conn_info, self.writer_port)

        mock_ssh = mock_ssh_client.return_value

        writer._add_firewalld_port(mock_ssh)
    ...
    @mock.patch('coriolis.utils.exec_ssh_cmd')
    @mock.patch('paramiko.SSHClient')
    def test__add_firewalld_port_with_exception(self, mock_ssh_client,
                                                mock_exec_ssh_cmd):
        writer = backup_writers.HTTPBackupWriterBootstrapper(
            self.ssh_conn_info, self.writer_port)
    ...
    @mock.patch('coriolis.utils.exec_ssh_cmd')
    @mock.patch('paramiko.SSHClient')
    def test__change_binary_se_context(self, mock_ssh_client,
                                       mock_exec_ssh_cmd):
        writer = backup_writers.HTTPBackupWriterBootstrapper(
            self.ssh_conn_info, self.writer_port)
        ...
        writer._change_binary_se_context(mock_ssh)
    ...
    @mock.patch('coriolis.utils.exec_ssh_cmd')
    @mock.patch('paramiko.SSHClient')
    def test__change_binary_se_context_with_exception(self, mock_ssh_client,
                                                      mock_exec_ssh_cmd):
        writer = backup_writers.HTTPBackupWriterBootstrapper(
            self.ssh_conn_info, self.writer_port)
    ...
    @mock.patch('coriolis.utils.exec_ssh_cmd')
    @mock.patch('paramiko.SSHClient')
    def test__copy_writer_file(self, mock_ssh_client, mock_exec_ssh_cmd):
        writer = backup_writers.HTTPBackupWriterBootstrapper(
            self.ssh_conn_info, self.writer_port)
        ...
        writer._copy_writer(mock_ssh)

        mock_sftp.stat.assert_called_once_with(writer._writer_cmd)
    ...
    def test__copy_writer_file_does_not_exist(
            self, mock_ssh_client, mock_exec_ssh_cmd,
            mock_get_resources_bin_dir):
        writer = backup_writers.HTTPBackupWriterBootstrapper(
            self.ssh_conn_info, self.writer_port)
        ...
        mock_exec_ssh_cmd.assert_has_calls([
            mock.call(
                mock_ssh, "sudo mv %s %s" % (
                    remote_tmp_path, writer._writer_cmd), get_pty=True
            ),
            mock.call(
                mock_ssh, "sudo chmod +x %s" % writer._writer_cmd,
                get_pty=True)])
        mock_sftp.close.assert_called_once()

    @mock.patch('coriolis.utils.exec_ssh_cmd')
    @mock.patch('paramiko.SSHClient')
    def test__copy_writer_stat_error(self, mock_ssh_client, mock_exec_ssh_cmd):
        writer = backup_writers.HTTPBackupWriterBootstrapper(
            self.ssh_conn_info, self.writer_port)
        ...
        self.assertRaises(IOError, writer._copy_writer, mock_ssh)

        mock_sftp.put.assert_not_called()
        mock_exec_ssh_cmd.assert_not_called()
        self.assertEqual(mock_sftp.close.call_count, 5)

    @mock.patch('coriolis.utils.read_ssh_file')
    @mock.patch('coriolis.utils.exec_ssh_cmd')
    @mock.patch('paramiko.SSHClient')
    def test__fetch_remote_file(self, mock_ssh_client, mock_exec_ssh_cmd,
                                mock_read_ssh_file):
        writer = backup_writers.HTTPBackupWriterBootstrapper(
            self.ssh_conn_info, self.writer_port)
        ...
        with mock.patch('builtins.open', mock.mock_open()) as data:
            writer._fetch_remote_file(
                mock_ssh, mock.sentinel.remote_file, mock.sentinel.local_file)
            data.assert_called_once_with(mock.sentinel.local_file, 'wb')
            mock_exec_ssh_cmd.assert_called_once_with(
                mock_ssh, "sudo chmod +r %s" % mock.sentinel.remote_file,
                get_pty=True)

            data().write.assert_called_once_with(
                mock_read_ssh_file.return_value)

    @mock.patch('coriolis.utils.test_ssh_path')
    @mock.patch('coriolis.utils.exec_ssh_cmd')
    @mock.patch('paramiko.SSHClient')
    def test__setup_certificates(
            self, mock_ssh_client, mock_exec_ssh_cmd, mock_test_ssh_path):
        writer = backup_writers.HTTPBackupWriterBootstrapper(
            self.ssh_conn_info, self.writer_port)
        ...
        result = writer._setup_certificates(mock_ssh)
    ...
    def test__setup_certificates_no_files_exist(
        ...
        writer._setup_certificates(mock_ssh)

        mock_exec_ssh_cmd.assert_any_call(
            mock_ssh, "sudo mkdir -p /etc/coriolis-writer", get_pty=True)
        mock_exec_ssh_cmd.assert_any_call(
            mock_ssh,
            "sudo %(writer_cmd)s generate-certificates -output-dir "
            "%(cert_dir)s -certificate-hosts %(extra_hosts)s" % {
                "writer_cmd": writer._writer_cmd,
                "cert_dir": "/etc/coriolis-writer",
                "extra_hosts": writer._ip,
            },
            get_pty=True)

    @mock.patch('coriolis.utils.exec_ssh_cmd')
    @mock.patch('paramiko.SSHClient')
    def test__read_remote_file_sudo(self, mock_ssh_client, mock_exec_ssh_cmd):
        writer = backup_writers.HTTPBackupWriterBootstrapper(
            self.ssh_conn_info, self.writer_port)

        mock_ssh = mock_ssh_client.return_value

        result = writer._read_remote_file_sudo(mock.sentinel.remote_path)

        mock_exec_ssh_cmd.assert_called_once_with(
            mock_ssh, "sudo cat %s" % mock.sentinel.remote_path, get_pty=True)
        self.assertEqual(
            result, mock_exec_ssh_cmd.return_value.decode.return_value)
    ...
    def test__init_writer(
            self, mock_ssh_client, mock_create_service,
            mock_add_firewalld_port, mock_inject_dport_allow_rule,
        ...
        writer._init_writer(mock_ssh, cert_paths)

        mock_change_binary_se_context.assert_called_once_with(mock_ssh)
        ...
        mock_inject_dport_allow_rule.assert_called_once_with(mock_ssh)
        mock_add_firewalld_port.assert_called_once_with(mock_ssh)
    ...
    @mock.patch.object(backup_writers, '_disable_lvm_metad_udev_rule')
    @mock.patch('paramiko.SSHClient')
    def test_setup_writer(self, mock_ssh_client, mock_disable_lvm_metad_udev,
        ...
        result = writer.setup_writer()

        mock_disable_lvm_metad_udev.assert_called_once_with(mock_ssh)
        mock_disable_lvm2_lvmetad.assert_called_once_with(mock_ssh)
        mock_copy_writer.assert_called_once_with(mock_ssh)
        mock_setup_certificates.assert_called_once_with(mock_ssh)
        mock_init_writer.assert_called_once_with(mock_ssh, cert_paths)

        expected_result = {
            "ip": writer._ip,
            "port": writer._writer_port,
            "certificates": {
...
class HTTPBackupWriterTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test_from_connection_info(self):
        ...
        writer = backup_writers.HTTPBackupWriter(
            conn_info["ip"], conn_info["port"], volumes_info, certificates)

        result = writer.from_connection_info(conn_info, volumes_info)
    ...
    def test_from_connection_info_missing_required_fields(self):
        ...
        writer = backup_writers.HTTPBackupWriter(
            conn_info["ip"], conn_info["port"], mock.sentinel.volumes_info,
            mock.sentinel.certificates)

        self.assertRaises(
            exception.CoriolisException, writer.from_connection_info,
    ...
    def test__del__(self, mock_isdir, mock_rmtree):
        mock_rmtree.return_value = None
        self.writer._crt_dir = mock.sentinel.crt_dir

        self.writer.__del__()

        mock_rmtree.assert_called_once_with(self.writer._crt_dir)

    @mock.patch('shutil.rmtree')
    @mock.patch('os.path.isdir')
    def test__del_with_exception(self, mock_isdir, mock_rmtree):
