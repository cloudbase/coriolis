...
class ClientTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test__test_connection_with_tunnel_no_exception(
        ...
        self.client._event_manager = mock.MagicMock()
        self.client._use_tunnel = True

        self.client._test_connection()

        mock_setup_tunnel.assert_called_once()
        self.client._event_manager.progress_update.assert_called_once_with(
    ...
    def test_test_connection_no_tunnel_with_exception(
        ...
        expected_calls = [
            mock.call("Testing direct connection to replicator (%s:%s)" % (
                self.ip, self.port)),
            mock.call(
                "Direct connection to replicator failed. Setting up tunnel."),
            mock.call("Testing connection to replicator (%s:%s)" % (
                self.ip, self.port)),
        ]

        self.client._event_manager.progress_update.assert_has_calls(
            expected_calls)
        mock_setup_tunnel.assert_called_once()
    ...
    def test__get_ssh_tunnel(self, mock_SSHTunnelForwarder):
        ...
        mock_SSHTunnelForwarder.assert_called_once_with((
            ...
            remote_bind_address=("127.0.0.1", self.ssh_conn_info["port"]),
    ...
    def test_get_status(self):
        ...
        result = self.client.get_status(device=self.device, brief=True)
    ...
    def test_get_status_no_device(self):
        ...
        result = self.client.get_status(device=None, brief=True)
    ...
    def test_get_chunks(self):
        ...
        result = self.client.get_chunks(self.device, skip_zeros=False)
    ...
    def test_get_changes(self):
        ...
        result = self.client.get_changes(self.device)
    ...
    def test_get_disk_size(self):
        ...
        result = self.client.get_disk_size(self.device)
    ...
    def test_download_chunk(self):
        ...
        result = self.client.download_chunk(self.disk, self.chunk)
...
class ReplicatorTestCase(test_base.CoriolisBaseTestCase):
    ...
    def setUp(self):
        ...
        with mock.patch.object(replicator_module.Replicator, '_setup_ssh',
                               return_value=None):
    ...
    def test__init_replicator_client(
        ...
        mock_Client.assert_called_once_with(
            ...
            use_compression=True, use_tunnel=False)
    ...
    @mock.patch('coriolis.utils.ignore_exceptions')
    def test__reconnect_ssh(self, mock_ignore_exceptions, mock_setup_ssh):
        self.replicator._ssh = self._ssh
        self.replicator._cert_dir = mock.MagicMock()

        result = self.replicator._reconnect_ssh()

        mock_ignore_exceptions.assert_called_once_with(self._ssh.close)
    ...
    def test_init_replicator(self, mock_init_replicator_client,
        ...
        mock_setup_replicator.assert_called_once()
    ...
    def test_wait_for_chunks(self, mock_get_status):
        ...
        self.replicator._event_manager.add_percentage_step.\
            assert_called_once_with(
                ...
                (mock.sentinel.disk, 1024.00), 100)
    ...
    def test__get_ssh_client(self, mock_ssh_client):
        ...
        result = self.replicator._get_ssh_client(self.conn_info)
    ...
    def test__get_ssh_client_exception(self, mock_ssh_client):
        ...
        self.assertRaises(exception.CoriolisException,
                          self.replicator._get_ssh_client, self.conn_info)
    ...
    def test__parse_source_ssh_conn_info_with_string_pkey(
        ...
        mock_deserialize_key.assert_called_once_with(
            "test_pkey",
            replicator_module.CONF.serialization.temp_keypair_password)
    ...
    def test__copy_file(self, mock_exec_ssh_cmd, mock_from_transport,
        ...
        self.replicator._copy_file(
            self._ssh, mock.sentinel.localPath, mock.sentinel.remotePath)
    ...
    def test_setup_replicator_group(self, mock_exec_ssh_cmd):
        ...
        self.assertEqual(result, bool(mock_exec_ssh_cmd.return_value))
    ...
    def test__setup_replicator_no_group(self, mock_exec_ssh_cmd):
        ...
        self.replicator._setup_replicator_group(
    ...
    def test__setup_certificates(self, mock_test_ssh_path, mock_isfile):
        ...
        result = self.replicator._setup_certificates(self._ssh, self.conn_info)
    ...
    def test_setup_certificates_no_files_exist(
        ...
        self.replicator._setup_certificates(self._ssh, self.conn_info)
        expected_calls = [
    ...
    def test__setup_replicator(
        ...
        mock_exec_ssh_cmd.assert_called_once_with(
            self._ssh,
            "sudo chmod 755 %s" %
            replicator_module.REPLICATOR_STATE, get_pty=True)
    ...
    @mock.patch.object(replicator_module.Client, 'download_chunk')
    @mock.patch.object(replicator_module.Client, 'get_changes')
    def test_replicate_disks(self, mock_get_changes, mock_download_chunk):
        self.replicator._cli.get_changes = mock_get_changes
        self.replicator._cli.download_chunk = mock_download_chunk
        ...
        self.backup_writer.open.assert_called_with("", "vol2")
        mock_download_chunk.assert_has_calls([
            mock.call("sdb1", {"length": 100, "offset": 0}),
            mock.call("sdb1", {"length": 200, "offset": 100}),
            mock.call("sdb2", {"length": 100, "offset": 0}),
            mock.call("sdb2", {"length": 200, "offset": 100})])

        self.assertEqual(result, self.replicator._repl_state)

    def test_replicate_disks_with_empty_state(self):
        self.replicator._repl_state = None

        self.replicator.replicate_disks(
            self.source_volumes_info, self.backup_writer)
    ...
    @mock.patch.object(replicator_module.Client, 'get_changes')
    @mock.patch.object(replicator_module.Client, 'get_status')
    @mock.patch.object(replicator_module.Client, 'get_chunks')
    def test_replicate_disks_initial_sync(
            self, mock_get_changes, mock_get_status, mock_get_chunks,
            mock_find_vol_state):
        mock_get_changes.return_value = [{
            'length': 100, 'offset': 0}, {'length': 200, 'offset': 100}]
        self.replicator._cli.get_status = mock_get_status
        self.replicator._cli.get_chunks = mock_get_chunks
        self.replicator._volumes_info = [{
        ...
        mock_find_vol_state.assert_called_with(
            "sdb", mock_get_status.return_value)
        mock_get_chunks.assert_called_with("sdb", skip_zeros=True)

    @mock.patch.object(replicator_module.Replicator, '_find_vol_state')
    @mock.patch.object(replicator_module.Client, 'get_changes')
    @mock.patch.object(replicator_module.Client, 'get_status')
    @mock.patch.object(replicator_module.Client, 'get_chunks')
    def test_replicate_disks_subsequent_sync(
            self, mock_get_changes, mock_get_status, mock_get_chunks,
            mock_find_vol_state):
        mock_get_changes.return_value = [{
            'length': 100, 'offset': 0}, {'length': 200, 'offset': 100}]
        ...
        mock_get_changes.assert_called_with("sdb")
    ...
    def test_replicate_disks_no_chunks(self):
        ...
        self.replicator._event_manager.progress_update.assert_called_with(
            "No new chunks to replicate for disk \"test_disk\" (sdb)"
        )
        self.assertEqual(result, self.replicator._repl_state)

    @mock.patch('builtins.open', new_callable=mock.mock_open)
    @mock.patch.object(replicator_module.Client, 'raw_disk_uri')
    @mock.patch.object(replicator_module.Client, 'get_disk_size')
    def test__download_full_disk(self, mock_get_disk_size,
                                 mock_raw_disk_uri, mock_open):
        mock_raw_disk_uri.return_value = mock.sentinel.uri
        self.replicator._cli.get_disk_size = mock_get_disk_size
        ...
        self.replicator._event_manager.progress_update.assert_called_with(
            "Downloading %s as thick file" % self.disk)
        self.replicator._event_manager.add_percentage_step.assert_called_with(
            "Downloading full disk /dev/%s" % self.disk,
            mock_get_disk_size.return_value)
    ...
    def test__download_sparse_disk(
            self, mock_get_size_from_chunks, mock_download_chunk,
            mock_get_disk_size, mock_open):
        self.replicator._cli.get_disk_size = mock_get_disk_size
        self.replicator._cli.download_chunk = mock_download_chunk
        ...
        file_handle.write.assert_called_once_with(
            mock_download_chunk.return_value)

        self.replicator._event_manager.add_percentage_step.assert_called_with(
            'Downloading spart disk /dev/test_disk (%s MB)' %
            mock_get_size_from_chunks.return_value, 1)
    ...
    @mock.patch.object(replicator_module.Client, 'get_chunks')
    def test_download_disk(self, mock_get_chunks, mock_download_sparse_disk,
    ...
    def test_download_disk_with_chunks(
        ...
        disk = "/dev/test_disk"
        self.replicator._cli.get_chunks = mock_get_chunks
        mock_get_chunks.return_value = [{'offset': 0, 'length': 100}]

        self.replicator.download_disk(disk, self.path)

        mock_get_chunks.assert_called_once_with(
            device=disk[5:], skip_zeros=True)
        mock_download_full_disk.assert_not_called()
        mock_download_sparse_disk.assert_called_once_with(
            disk[5:], self.path, mock_get_chunks.return_value)
