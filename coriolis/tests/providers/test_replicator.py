...
class SSHTunnelTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test_start(self, mock_connect_ssh, mock_socket, mock_thread):
        ...
        self.assertEqual(mock_client.get_transport.return_value, self.tunnel._transport)

        mock_socket.assert_called_once_with(
            replicator_module.socket.AF_INET, replicator_module.socket.SOCK_STREAM
        )
        mock_sock.setsockopt.assert_called_once_with(
            replicator_module.socket.SOL_SOCKET,
            replicator_module.socket.SO_REUSEADDR,
            1,
        )
        mock_sock.bind.assert_called_once_with(self.tunnel._requested_local)
    ...
    def test_forward(self, mock_select):
        ...
        self.tunnel._transport.open_channel.assert_called_once_with(
            'direct-tcpip',
            self.tunnel._remote_bind_address,
    ...
    def test_stop(self):
        ...
        self.tunnel._transport = mock.MagicMock()
        self.tunnel._transport.close.side_effect = Exception("boom goes the dynamite")
...
class ClientTestCase(test_base.CoriolisBaseTestCase):
    ...
    def setUp(self):
        ...
        with (
            mock.patch.object(
                replicator_module.Client, '_get_session', return_value=None
            ),
            mock.patch.object(
                replicator_module.Client, '_test_connection', return_value=None
            ),
        ):
    ...
    def test_base_uri(self):
        self.client._ip_via_tunnel = self.ip_via_tunnel
        self.client._port_via_tunnel = self.port_via_tunnel
        expected_uri_via_tunnel = "https://%s:%s" % (
            self.ip_via_tunnel,
    ...
    def test__setup_tunnel_connection(self, mock_get_ssh_tunnel):
        ...
        mock_tunnel.local_bind_address = (self.ip_via_tunnel, self.port_via_tunnel)
    ...
    def test__test_connection_no_tunnel_no_exception(self, mock_wait_for_port):
        ...
        mock_wait_for_port.assert_called_once_with(self.ip, self.port, max_wait=30)
    ...
    def test_test_connection_with_tunnel_with_exception(
        ...
        with self.assertLogs('coriolis.providers.replicator', logging.WARNING):
    ...
    def test__get_ssh_tunnel_no_credentials(self):
        ...
        self.assertRaises(exception.CoriolisException, self.client._get_ssh_tunnel)
    ...
    def test__get_session(self, mock_Session):
        ...
        self.assertEqual(
            result.cert,
            (self.client._creds["client_cert"], self.client._creds["client_key"]),
        )
    ...
    def test_get_status(self):
        ...
        original_get_status = testutils.get_wrapped_function(self.client.get_status)

        result = original_get_status(self.client, device=self.device, brief=True)
    ...
    def test_get_chunks(self):
        ...
        original_get_chunks = testutils.get_wrapped_function(self.client.get_chunks)

        result = original_get_chunks(self.client, self.device, skip_zeros=False)

        self.assertEqual(result, self.mock_response.json.return_value)
        self.client._cli.get.assert_called_once_with(
            "https://%s:%s/api/v1/dev/%s/chunks/" % (self.ip, self.port, self.device),
    ...
    def test_get_changes(self):
        ...
        original_get_changes = testutils.get_wrapped_function(self.client.get_changes)
    ...
    def test_get_disk_size(self):
        ...
        self.assertEqual(result, int(self.mock_response.headers["Content-Length"]))
    ...
    def test_get_disk_checksum(self):
        ...
        self.assertEqual(result, self.mock_response.json.return_value)
        self.client._cli.get.assert_called_once_with(
            "https://%s:%s/api/v1/dev/%s/checksum" % (self.ip, self.port, self.device),
            timeout=replicator_module.CONF.replicator.default_requests_timeout,
        )
        self.mock_response.raise_for_status.assert_called_once()
...
class ReplicatorTestCase(test_base.CoriolisBaseTestCase):
    ...
    def setUp(self):
        ...
        with (
            mock.patch.object(replicator_module, 'CONF', return_value=mock.MagicMock()),
            mock.patch.object(
                replicator_module.Replicator, '_setup_ssh', return_value=None
            ),
        ):
    ...
    @mock.patch.object(replicator_module.Replicator, '_parse_source_ssh_conn_info')
    @mock.patch.object(replicator_module.Replicator, '_parse_replicator_conn_info')
    @mock.patch.object(replicator_module, 'Client')
    def test__init_replicator_client(
        self,
        mock_Client,
        mock_parse_replicator_conn_info,
        ...
        mock_Client.assert_called_once_with(
            ...
            self.conn_info,
            self.conn_info,
            self.replicator._event_manager,
    ...
    @mock.patch.object(replicator_module.Replicator, '_parse_source_ssh_conn_info')
    def test__setup_ssh(self, mock_parse_source_ssh_conn_info, mock_get_ssh_client):
        mock_parse_source_ssh_conn_info.return_value = self.conn_info
    ...
    def test_init_replicator(self, mock_init_replicator_client, mock_setup_replicator):
    ...
    def test_attach_new_disk_missing_device_paths(self, mock_get_status):
        ...
        with self.assertLogs('coriolis.providers.replicator', logging.WARN):
    ...
    def test_attach_new_disk_no_new_device_paths(self, _, mock_get_status):
        ...
        self.replicator._volumes_info = [{'disk_id': 'disk1', 'disk_path': 'path1'}]
    ...
    def test_attach_new_disk_new_device_paths_after_retry(self, _, mock_get_status):
        ...
        mock_return_value_before = [{'device-path': 'path1'}]
        mock_return_value_after = [{'device-path': 'path1'}, {'device-path': 'path2'}]
        ...
        self.replicator.attach_new_disk('disk1', attachf, retry_count=2, retry_period=0)
    ...
    def test_wait_for_chunks(self, mock_get_status):
        ...
        self.replicator._event_manager.add_percentage_step.return_value = (
            mock.sentinel.perc_step
        )
    ...
    def test_wait_for_chunks_not_initialized(self):
        ...
        self.assertRaises(exception.CoriolisException, self.replicator.wait_for_chunks)
    ...
    def test__get_ssh_client_exception(self, mock_ssh_client):
        ...
        self.assertRaises(
            exception.CoriolisException, original_get_ssh_client, self.replicator, arg
        )
    ...
    def test__copy_file(self, mock_exec_ssh_cmd, mock_from_transport, mock_mktemp):
        mock_sftp = mock.MagicMock()
        mock_from_transport.return_value = mock_sftp

        original_copy_file = testutils.get_wrapped_function(self.replicator._copy_file)

        original_copy_file(
            self.replicator,
            self._ssh,
            mock.sentinel.localPath,
            mock.sentinel.remotePath,
        )

        mock_mktemp.assert_called_once_with(dir='/tmp')
        mock_from_transport.assert_called_once_with(self._ssh.get_transport())
        ...
        mock_exec_ssh_cmd.assert_called_once_with(
            self._ssh,
            "sudo mv %s %s" % (mock_mktemp.return_value, mock.sentinel.remotePath),
            get_pty=False,
        )
    ...
    def test__copy_replicator_cmd(
        self, mock_exec_ssh_cmd, mock_copy_file, mock_get_resources_bin_dir, mock_join
    ):
        ...
        mock_copy_file.assert_called_once_with(
            self._ssh, mock_join.return_value, replicator_module.REPLICATOR_PATH
        )
        mock_exec_ssh_cmd.assert_called_once_with(
            self._ssh,
            "sudo chmod +x %s" % replicator_module.REPLICATOR_PATH,
            get_pty=False,
        )
        ...
        mock_exec_ssh_cmd.assert_has_calls(
            [
                mock.call(
                    ...
                    "echo 1 || echo 0" % replicator_module.REPLICATOR_GROUP_NAME,
                ),
                mock.call(self._ssh, "sudo groupadd %s" % group_name, get_pty=False),
                mock.call(
                    self._ssh,
                    "sudo usermod -aG %s %s"
                    % (
        ...
        mock_exec_ssh_cmd.assert_has_calls(
            [
                mock.call(
                    ...
                    "echo 1 || echo 0" % replicator_module.REPLICATOR_USERNAME,
                ),
                mock.call(
                    self._ssh,
                ...
                mock.call(
                    self._ssh,
                    "sudo usermod -aG disk %s" % replicator_module.REPLICATOR_USERNAME,
                    get_pty=False,
                ),
            ]
        )
    ...
    def test__exec_replicator_cmd(self, mock_create_service):
        ...
        self.replicator._exec_replicator(
            self._ssh, self.conn_info['port'], certs, mock.sentinel.state
        )
    ...
    def test__setup_certificates(self, mock_test_ssh_path, mock_isfile):
        ...
        result = original_setup_certificates(self.replicator, self._ssh, self.conn_info)
    ...
    def test_setup_certificates_no_files_exist(
        self, mock_exec_ssh_cmd, mock_test_ssh_path, mock_fetch_remote_file
    ):
        ...
        original_setup_certificates(self.replicator, self._ssh, self.conn_info)

        expected_calls = [
            mock.call(
                self._ssh,
                "sudo mkdir -p %s" % replicator_module.REPLICATOR_DIR,
                get_pty=False,
            ),
            mock.call(
                self._ssh,
                "sudo %s gen-certs -output-dir" % replicator_module.REPLICATOR_PATH
            ...
            mock.call(
                ...
                % (
                    replicator_module.REPLICATOR_USERNAME,
                    replicator_module.REPLICATOR_GROUP_NAME,
                    replicator_module.REPLICATOR_DIR,
                ),
                get_pty=False,
            ),
            mock.call(
                self._ssh,
                "sudo chmod -R g+r %s" % replicator_module.REPLICATOR_DIR,
                get_pty=False,
            ),
        ]
    ...
    @mock.patch.object(replicator_module.Replicator, '_get_replicator_state_file')
    @mock.patch.object(replicator_module.Replicator, '_copy_file')
    @mock.patch.object(replicator_module.utils, 'exec_ssh_cmd')
    @mock.patch.object(replicator_module.os, 'remove')
    @mock.patch.object(replicator_module.Replicator, '_parse_replicator_conn_info')
    ...
    def test__setup_replicator(
        self,
        mock_exec_replicator,
        mock_setup_certificates,
        mock_setup_replicator_user,
        mock_reconnect_ssh,
        mock_setup_replicator_group,
        mock_copy_replicator_cmd,
        mock_parse_replicator_conn_info,
        mock_os_remove,
        mock_exec_ssh_cmd,
        mock_copy_file,
        ...
        mock_exec_ssh_cmd.assert_has_calls(
            [
                ...
                mock.call(
                    self._ssh, "sudo chcon -t bin_t /usr/bin/replicator", get_pty=False
                ),
            ]
        )
        ...
        mock_setup_replicator_group.assert_called_once_with(
            self._ssh, group_name=replicator_module.REPLICATOR_GROUP_NAME
        )
        ...
        mock_setup_certificates.assert_called_once_with(
            self._ssh, mock_parse_replicator_conn_info.return_value
        )
    ...
    def test__change_binary_se_context_with_exception(self, mock_exec_ssh_cmd):
        ...
        with self.assertLogs('coriolis.providers.replicator', level=logging.WARN):
            self.replicator._change_binary_se_context(self._ssh)
    ...
    def test__find_vol_state(self):
        ...
        result = self.replicator._find_vol_state(mock.sentinel.device_name, state)
    ...
    @mock.patch.object(replicator_module, 'Client')
    def test_replicate_disks_calls_verify_checksum(self, mock_Client, mock_verify):
        self.replicator._cli = mock_Client.return_value
        self.replicator._cli.get_changes.return_value = [{'length': 100, 'offset': 0}]
        self.replicator._volumes_info = [
            {"disk_id": "test_disk", "disk_path": "/dev/sdb"}
        ]
        source_volumes_info = [{"disk_id": "test_disk", "disk_path": "/dev/sdb"}]
        self.replicator._repl_state = ['non-empty']
        mock_destination = mock.MagicMock(autospec=backup_writers.BaseBackupWriterImpl)
        self.backup_writer.open.return_value.__enter__.return_value = mock_destination

        self.replicator.replicate_disks(source_volumes_info, self.backup_writer, True)
    ...
    def test_replicate_disks_with_nonexistent_volume(self):
        ...
        source_volumes_info = [
            {"disk_id": "vol4", "disk_path": "/dev/sdb4", "other-data": "data4"},
        ]
    ...
    @mock.patch.object(replicator_module, 'Client')
    def test_replicate_disks_initial_sync(
        self, mock_Client, mock_find_vol_state, mock_verify
    ):
        self.replicator._cli = mock_Client.return_value
        ...
        self.replicator._volumes_info = [{"disk_id": "test_disk", "zeroed": True}]
        source_volumes_info = [
            {"disk_id": "test_disk", "disk_path": "/dev/sdb", "zeroed": True}
        ]
        self.replicator._repl_state = None

        self.replicator.replicate_disks(source_volumes_info, self.backup_writer)

        mock_find_vol_state.assert_called_with(
            "sdb", self.replicator._cli.get_status.return_value
        )
        self.replicator._cli.get_chunks.assert_called_with("sdb", skip_zeros=True)

    def test_replicate_disks_no_chunks(self):
        self.replicator._cli.get_changes.return_value = []
        self.replicator._volumes_info = [{"disk_id": "test_disk", "zeroed": False}]
        source_volumes_info = [{"disk_id": "test_disk", "disk_path": "/dev/sdb"}]
    ...
    def test__download_full_disk(self, mock_Client, mock_open):
        ...
        self.replicator._cli.get_chunks.return_value = [{'offset': 0, 'length': 100}]
        self.replicator._cli._cli.get.return_value.__enter__.return_value.iter_content.return_value = [
            b'chunk1',
            b'chunk2',
            b'chunk3',
        ]
        ...
        file_handle.write.assert_has_calls(
            [mock.call(b'chunk1'), mock.call(b'chunk2'), mock.call(b'chunk3')]
        )
        self.replicator._event_manager.set_percentage_step.assert_has_calls(
            [
    ...
    def test__download_sparse_disk(
        ...
        self.replicator._download_sparse_disk(self.disk, mock.sentinel.path, chunks)
    ...
    @mock.patch.object(replicator_module, 'Client')
    def test_download_disk(
        self, mock_Client, mock_download_sparse_disk, mock_download_full_disk
    ):
        self.replicator._cli = mock_Client.return_value
        disk = "/dev/test_disk"
        ...
        mock_download_full_disk.assert_called_once_with(self.disk, self.path)
    ...
    def test_download_disk_with_chunks(
        self, mock_download_sparse_disk, mock_download_full_disk, mock_Client
    ):
        self.replicator._cli = mock_Client.return_value
