...
from coriolis.osmorphing.osmount import base, factory
...
class GetOsMountToolsTestCase(test_base.CoriolisBaseTestCase):
    ...
    @mock.patch.object(
        factory.ubuntu.UbuntuOSMountTools, 'check_os', return_value=False
    )
    @mock.patch.object(
        factory.redhat.RedHatOSMountTools, 'check_os', return_value=False
    )
    @mock.patch.object(factory.suse.SUSEOSMountTools, 'check_os', return_value=False)
    @mock.patch.object(
        factory.windows.WindowsMountTools, 'check_os', return_value=False
    )
    def test_get_os_mount_tools_no_os_found(
        ...
        self.assertRaises(
            exception.CoriolisException,
            factory.get_os_mount_tools,
            factory.constants.OS_TYPE_LINUX,
            mock_connect,
            mock.sentinel.event_manager,
            mock.sentinel.ignore_devices,
            mock.sentinel.operation_timeout,
    ...
    @mock.patch.object(base.BaseSSHOSMountTools, '_connect')
    @mock.patch.object(base.BaseSSHOSMountTools, '_exec_cmd')
    @mock.patch.object(factory.ubuntu.UbuntuOSMountTools, 'check_os', return_value=True)
    @mock.patch.object(
        factory.redhat.RedHatOSMountTools, 'check_os', return_value=False
    )
    @mock.patch.object(factory.suse.SUSEOSMountTools, 'check_os', return_value=False)
