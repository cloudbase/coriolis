...
class BaseSSHOSMountToolsTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test__connect(
        ...
        mock_wait_for_port_connectivity,
        mock_ssh_client,
    ):
        ...
        original_connect = testutils.get_wrapped_function(base_os_mount_tools._connect)

        with self.assertLogs('coriolis.osmorphing.osmount.base', level=logging.INFO):
        ...
        self.ssh.set_log_channel.assert_called_once_with(
            "paramiko.morpher.%s.%s" % (self.conn_info['ip'], 22)
        )

    @mock.patch.object(base.utils, 'check_env_command')
    @mock.patch.object(base.BaseSSHOSMountTools, '_allow_ssh_env_vars')
    @mock.patch.object(base.BaseSSHOSMountTools, '_connect')
    def test_setup(self, mock_connect, mock_allow_ssh_vars, mock_check_env_command):
    ...
    def test_setup_without_env_command(
        ...
        self.assertRaises(exception.CoriolisException, self.base_os_mount_tools.setup)
    ...
    def test__exec_cmd(self, mock_exec_ssh_cmd):
        ...
        mock_exec_ssh_cmd.assert_called_once_with(
            self.base_os_mount_tools._ssh, self.cmd, {}, get_pty=False, timeout=120
        )
    ...
    def test__exec_cmd_with_exception(self, mock_exec_ssh_cmd):
        ...
        self.assertRaises(
            ...
            self.base_os_mount_tools._exec_cmd,
            self.cmd,
            timeout=self.base_os_mount_tools._osmount_operation_timeout,
        )
    ...
    def test__exec_sudo_env_cmd_with_proxy(self, mock_exec_ssh_cmd):
        ...
        self.assertEqual(
            ...
            "HTTPS_PROXY=http://10.0.0.1:3128 apt-get update -y",
            cmd,
        )
    ...
    def test__exec_sudo_env_cmd_non_proxy_environment(self, mock_exec_ssh_cmd):
        ...
        self.base_os_mount_tools._exec_sudo_env_cmd("apt-get install cryptsetup -y")
    ...
    def test__exec_sudo_env_cmd_without_proxy(self, mock_exec_ssh_cmd):
        ...
        self.base_os_mount_tools._environment = {"DEBIAN_FRONTEND": "noninteractive"}

        self.base_os_mount_tools._exec_sudo_env_cmd("apt-get install cryptsetup -y")

        self.assertEqual(
            "sudo env DEBIAN_FRONTEND=noninteractive apt-get install cryptsetup -y",
            mock_exec_ssh_cmd.call_args[0][1],
        )

    @mock.patch.object(base.utils, 'exec_ssh_cmd')
    def test__exec_sudo_env_cmd_quotes_sensitive_proxy_values(self, mock_exec_ssh_cmd):
        ...
        self.assertEqual(
            ["sudo", "env", "http_proxy=%s" % proxy, "apt-get", "update", "-y"],
...
class BaseLinuxOSMountToolsTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test__get_pvs(self, mock_exec_cmd):
        mock_exec_cmd.return_value = "pv1:vg1\nimproper_line\npv2:vg1\n\npv3:vg2"

        with self.assertLogs('coriolis.osmorphing.osmount.base', level=logging.WARN):
    ...
    def test__get_vgs(self, mock_exec_cmd):
        ...
        expected_result = {
            "uuid1": {"name": "vg1", "pvs": ["pv1"]},
            "uuid2": {"name": "vg2", "pvs": ["pv3"]},
        }
    ...
    def test__get_vgs_duplicate_vg_names(self, mock_exec_cmd, mock_uuid4):
        ...
        with self.assertLogs('coriolis.osmorphing.osmount.base', level=logging.DEBUG):
            result = self.base_os_mount_tools._get_vgs()

        vgs_cmd = "sudo vgs -o vg_name,pv_name,vg_uuid, --noheadings --separator :"
        mock_exec_cmd.assert_has_calls(
            [mock.call(vgs_cmd), mock.call("sudo vgrename uuid2 random_uuid")]
        )

        expected_result = {
            "uuid1": {"name": "vg1", "pvs": ["pv1"]},
            "uuid2": {"name": "random_uuid", "pvs": ["pv2"]},
        }
    ...
    def test__check_vgs_with_exception(self, mock_exec_cmd):
        ...
        self.assertRaises(
            exception.CoriolisException, self.base_os_mount_tools._check_vgs
        )
    ...
    def test__check_mount_fstab_partitions(
        self,
        mock_get_device_file_paths,
        mock_exec_cmd,
        mock_read_ssh_file,
        mock_test_ssh_path,
    ):
        ...
        mock_exec_cmd.return_value = b"/dev/sda1:UUID=uuid1\n/dev/sda2:UUID=uuid2"
        mock_read_ssh_file.return_value = (
            b"Unparseable line\nUUID=uuid2 /mnt1 ext4 defaults 0 0"
        )
        ...
        mock_exec_cmd.assert_called_once_with(
            "sudo mount -t ext4 -o defaults /dev/disk/by-uuid/uuid2 '/root/mnt1'"
        )
        ...
        mock_test_ssh_path.assert_has_calls(
            [
                ...
                mock.call(self.base_os_mount_tools._ssh, "/dev/disk/by-uuid/uuid2"),
            ]
        )
    ...
    def test__check_mount_fstab_partitions_with_label(
        ...
        mock_test_ssh_path.return_value = True
        mock_get_device_file_paths.return_value = ["/dev/sda1", "/dev/sda2"]
        mock_exec_cmd.return_value = b"/dev/sda1:LABEL=boot\n/dev/sda2:LABEL=UEFI"
        mock_read_ssh_file.return_value = b"LABEL=UEFI /boot/efi vfat defaults 0 0"
        ...
        mock_get_device_file_paths.assert_called_once()
        mock_exec_cmd.assert_called_once_with(
            "sudo mount -t vfat -o defaults /dev/disk/by-label/UEFI '/root/boot/efi'"
        )
        mock_read_ssh_file.assert_called_once_with(
            self.base_os_mount_tools._ssh, mocked_full_path
        )
        mock_test_ssh_path.assert_has_calls(
            [
                mock.call(self.base_os_mount_tools._ssh, mocked_full_path),
                mock.call(self.base_os_mount_tools._ssh, "/dev/disk/by-label/UEFI"),
            ]
        )
    ...
    def test__check_mount_fstab_partitions_no_fstab(self, mock_test_ssh_path):
        ...
        with self.assertLogs('coriolis.osmorphing.osmount.base', level=logging.WARNING):
            result = self.base_os_mount_tools._check_mount_fstab_partitions(
                self.os_root_dir
            )
    ...
    def test__check_mount_fstab_partitions_unsupported_device(
        ...
        mock_get_device_file_paths.return_value = ["/dev/sda1", "/dev/sda2"]
        mock_exec_cmd.return_value = b"/dev/sda1:UUID=uuid1\n/dev/sda2:UUID=uuid2"
        mock_read_ssh_file.return_value = b"/dev/sda3 /mnt1 ext4 defaults 0 0"

        with self.assertLogs('coriolis.osmorphing.osmount.base', level=logging.WARN):
            result = self.base_os_mount_tools._check_mount_fstab_partitions(
                self.os_root_dir, mountable_lvm_devs=["/dev/sda1", "/dev/sda2"]
            )
            self.assertEqual(result, [])

        mock_get_device_file_paths.assert_called_once()
        mock_exec_cmd.assert_called_once_with("readlink -en /dev/sda3")
        mock_read_ssh_file.assert_called_once_with(
            self.base_os_mount_tools._ssh, mocked_full_path
        )
        mock_test_ssh_path.assert_called_once_with(
    ...
    def test__check_mount_fstab_partitions_skip_undesired_mount(
        ...
        mock_get_device_file_paths.return_value = ["/dev/sda1", "/dev/sda2"]
        mock_exec_cmd.return_value = b"/dev/sda1:UUID=uuid1\n/dev/sda2:UUID=uuid2"
        mock_read_ssh_file.return_value = b"UUID=uuid2 /mnt1 ext4 defaults 0 0"
    ...
    def test__check_mount_fstab_partitions_skip_filesystems(
        ...
        mock_get_device_file_paths.return_value = ["/dev/sda1", "/dev/sda2"]
        mock_exec_cmd.return_value = b"/dev/sda1:UUID=uuid1\n/dev/sda2:UUID=uuid2"
        mock_read_ssh_file.return_value = b"UUID=uuid1 /mnt1 ext4 defaults 0 0"
    ...
    def test__check_mount_fstab_partitions_mountcmd_with_exception(
        ...
        with self.assertLogs('coriolis.osmorphing.osmount.base', level=logging.WARNING):
            self.base_os_mount_tools._check_mount_fstab_partitions(self.os_root_dir)
        ...
        mock_test_ssh_path.assert_has_calls(
            [
                mock.call(self.base_os_mount_tools._ssh, mocked_full_path),
                mock.call(self.base_os_mount_tools._ssh, "/dev/disk/by-uuid/uuid1"),
            ]
        )
    ...
    def test__get_symlink_target(self, mock_exec_cmd):
        ...
        result = self.base_os_mount_tools._get_symlink_target("/dev/sda1")

        mock_exec_cmd.assert_called_once_with('readlink -en %s' % "/dev/sda1")
    ...
    def test__get_symlink_target_with_exception(self, mock_exec_cmd):
        ...
        with self.assertLogs('coriolis.osmorphing.osmount.base', level=logging.WARN):
            self.base_os_mount_tools._get_symlink_target("/dev/sda1")
    ...
    def test__get_device_file_paths(self, mock_get_symlink_target):
        ...
        result = self.base_os_mount_tools._get_device_file_paths(symlink_list)
        ...
        mock_get_symlink_target.assert_has_calls(
            [mock.call('/dev/GROUP/VOLUME0'), mock.call('/dev/mapper/GROUP-VOLUME0')]
        )
    ...
    @mock.patch.object(base.BaseSSHOSMountTools, '_exec_cmd')
    @mock.patch.object(base.utils, 'test_ssh_path')
    def test__get_mounted_devices_not_found(self, mock_test_ssh_path, mock_exec_cmd):
        mock_exec_cmd.side_effect = [
            "/dev/sda1 on / type ext4 (rw,relatime,errors=remount-ro)\n",
            "/dev/sda1",
            "",
    ...
    @mock.patch.object(base.BaseSSHOSMountTools, '_exec_cmd')
    @mock.patch.object(base.utils, 'list_ssh_dir')
    def test__find_dev_with_contents_one_of_files(
        self, mock_list_ssh_dir, mock_exec_cmd
    ):
        mock_exec_cmd.return_value = "/tmp/tmp_dir"
        mock_list_ssh_dir.return_value = ["etc", "bin", "sbin", "boot"]
    ...
    @mock.patch.object(base.BaseSSHOSMountTools, '_exec_cmd')
    @mock.patch.object(base.utils, 'list_ssh_dir')
    def test__find_dev_with_contents_missing_all_files(
        self, mock_list_ssh_dir, mock_exec_cmd
    ):
        mock_exec_cmd.return_value = "/tmp/tmp_dir"
        mock_list_ssh_dir.return_value = ["etc", "bin", "sbin", "boot"]
    ...
    @mock.patch.object(base.BaseSSHOSMountTools, '_exec_cmd')
    @mock.patch.object(base.utils, 'list_ssh_dir')
    def test__find_dev_with_contents_with_exception(
        self, mock_list_ssh_dir, mock_exec_cmd
    ):
        mock_exec_cmd.side_effect = [
            b"/tmp/tmp_dir",
    ...
    def test__find_and_mount_root(
        self, mock_find_dev_with_contents, mock_exec_cmd, mock_test_ssh_path
    ):
    ...
    def test__find_and_mount_root_with_exception(
        ...
        self.assertRaises(
            exception.OperatingSystemNotFound,
    ...
    def test__mask_minion_efi_firmware_bios_nested_params(
        ...
        self.base_os_mount_tools._osmorphing_info = {
            "osmorphing_parameters": {"firmware_type": constants.FIRMWARE_TYPE_BIOS}
        }
        mock_test_ssh_path.return_value = True
    ...
    def test_mount_os(
        ...
        mock_get_vgs.return_value = {"vgid1": {"name": "vg1", "pvs": ["/dev/sda1"]}}
    ...
    def test__find_and_mount_root_severs_mount_propagation(
        ...
        for directory in ['proc', 'sys', 'dev', 'run']:
            ...
            self.assertEqual(issued.index(private_cmd), issued.index(bind_cmd) + 1)
    ...
    def test_set_proxy(self, mock_get_url_with_credentials):
        ...
        self.assertEqual(
            self.base_os_mount_tools._environment['no_proxy'], 'cloudbase.it.127.0.0.1'
        )

    @mock.patch.object(base.utils, 'get_url_with_credentials')
    def test_set_proxy_no_url(self, mock_get_url_with_credentials):
        proxy_settings = {'username': "admin", 'password': 'Random-Password-123!'}
