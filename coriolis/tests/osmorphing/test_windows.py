...
class BaseWindowsMorphingToolsTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test_get_required_detected_os_info_fields(self):
        result = self.morphing_tools.get_required_detected_os_info_fields()
    ...
    def test__get_dism_path(self, mock_get_worker_os_drive_path):
        ...
        self.assertEqual(
            result,
            "%s\\Windows\\System32\\dism.exe" %
            mock_get_worker_os_drive_path.return_value
        )
    ...
    def test__get_ps_fl_value(self, mock_search):
        ...
        mock_search.assert_called_once_with(
            r"^%s\s*: (.*)$" % mock.sentinel.name, mock.sentinel.data,
            windows.re.MULTILINE)

        self.assertEqual(
            result, mock_search.return_value.groups.return_value[0])

    @mock.patch.object(windows.BaseWindowsMorphingTools, '_get_dism_path')
    @mock.patch.object(
        windows.BaseWindowsMorphingTools, '_get_worker_os_drive_path'
    )
    def test__add_dism_driver(self, mock_get_worker_os_drive_path,
        ...
        self.conn.exec_command.assert_called_once_with(
            mock_get_dism_path.return_value,
            ['/add-driver', '/image:%s' % self.os_root_dir,
             '/driver:"%s"' % mock.sentinel.driver_path,
             '/recurse', '/forceunsigned'])
        mock_get_dism_path.assert_called_once_with()
        mock_get_worker_os_drive_path.assert_not_called()
    ...
    def test_run_user_script_raises_exception_on_exec_ps_command(
        ...
        mock_write_winrm_file.assert_called_once_with(
            self.conn, mock.ANY, user_script)
    ...
    def test__get_cbslinit_scripts_dir(self):
        result = self.morphing_tools._get_cbslinit_scripts_dir(
            mock.sentinel.base_dir)

        self.assertEqual(result, "%s\\LocalScripts" % mock.sentinel.base_dir)

    @mock.patch.object(
        windows.BaseWindowsMorphingTools, '_get_cbslinit_scripts_dir'
    )
    @mock.patch.object(windows.utils, 'write_winrm_file')
    @mock.patch('os.path')
    def test__write_local_script(self, mock_os_path,
                                 mock_write_winrm_file,
                                 mock_get_cbslinit_scripts_dir):
        priority = 50

        with mock.patch('builtins.open', mock.mock_open()) as mock_open:
            self.morphing_tools._write_local_script(
                mock.sentinel.base_dir, mock.sentinel.script_path,
                priority=priority)
            mock_open.assert_called_once_with(
                mock.sentinel.script_path, 'r')
            mock_write_winrm_file.assert_called_once_with(
                self.conn,
                "%s\\%d-%s" % (
                    mock_get_cbslinit_scripts_dir.return_value,
                    priority, mock_os_path.basename.return_value), '')

        mock_get_cbslinit_scripts_dir.assert_called_once_with(
            mock.sentinel.base_dir)
    ...
    def test__write_cloudbase_init_conf(
            self, mock_os_path, mock_write_local_script,
            mock_write_winrm_file, mock_get_cbslinit_scripts_dir):
        ...
        conf_content = (
            ...
            {"bin_path": "%s\\Bin" % mock.sentinel.local_base_dir,
             "log_path": "%s\\Log" % mock.sentinel.local_base_dir,
             "scripts_path": "%s\\LocalScripts" % mock.sentinel.local_base_dir,
             "com_port": 'COM1',
        ...
        mock_write_local_script.assert_called_once_with(
            mock.sentinel.cloudbaseinit_base_dir,
            mock_os_path.join.return_value, priority=99)
    ...
    def test__write_cloudbase_init_conf_with_exception(
        ...
        mock_get_cbslinit_scripts_dir.assert_not_called()
        self.conn.exec_ps_command.assert_not_called()
        mock_write_winrm_file.assert_not_called()
    ...
    def test__install_cloudbase_init(
            ...
            mock_unload_registry_hive, mock_get_cbslinit_base_dir):
        cloudbaseinit_zip_path = 'c:\\cloudbaseinit.zip'
        local_base_dir = "C%s" % mock_get_cbslinit_base_dir.return_value[1:]
        ...
        mock_get_cbslinit_base_dir.assert_called_once_with()
        mock_load_registry_hive.assert_called_once_with(
            "HKLM\\%s" % mock_uuid4.return_value,
            "%sWindows\\System32\\config\\SYSTEM" % self.os_root_dir)

        self.event_manager.progress_update.assert_has_calls([
            mock.call("Adding cloudbase-init"),
            mock.call("Downloading cloudbase-init"),
            mock.call("Installing cloudbase-init"),
            mock.call("Enabling cloudbase-init"),
        ])
        mock_expand_archive.assert_called_once_with(
            cloudbaseinit_zip_path, mock_get_cbslinit_base_dir.return_value)

        self.conn.exec_ps_command.assert_called_once_with(
            "mkdir '%s' -Force" %
            ("%s\\Log" % mock_get_cbslinit_base_dir.return_value),
            ignore_stdout=True)

        mock_write_cloudbase_init_conf.assert_called_once_with(
            mock_get_cbslinit_base_dir.return_value, local_base_dir,
        ...
        self.assertEqual(result, mock_get_cbslinit_base_dir.return_value)
    ...
    def test_install_cloudbase_init_existing_service(
            ...
            mock_setup_existing_cbslinit_service,
            mock_check_cloudbase_init_exists, mock_get_cbslinit_base_dir,
    ...
    @mock.patch.object(windows.utils, 'write_winrm_file')
    @mock.patch.object(windows.json, 'dumps')
    @mock.patch('base64.b64encode')
    def test__write_static_ip_script(self, mock_b64encode, mock_json_dumps,
                                     mock_write_winrm_file,
                                     mock_get_cbslinit_scripts_dir):
        self.morphing_tools._write_static_ip_script(
            mock.sentinel.base_dir, mock.sentinel.nics_info,
            mock.sentinel.ips_info)

        contents = windows.STATIC_IP_SCRIPT_TEMPLATE % {
            'nics_info': mock_b64encode.return_value.decode.return_value,
            'ips_info': mock_b64encode.return_value.decode.return_value
        }
        mock_get_cbslinit_scripts_dir.assert_called_once_with(
            mock.sentinel.base_dir)
        mock_write_winrm_file.assert_called_once_with(
            self.morphing_tools._conn,
            "%s\\01-static-ip-config.ps1" %
            mock_get_cbslinit_scripts_dir.return_value,
            contents.encode())
    ...
    @mock.patch.object(windows.uuid, 'uuid4')
    def test_set_net_config(self, mock_uuid4, mock_check_ips_info,
                            mock_load_registry_hive,
                            mock_unload_registry_hive,
        ...
        self.morphing_tools.set_net_config(mock.sentinel.nics_info, dhcp=dhcp)
        ...
        mock_compile_static_ip_conf_from_registry.assert_called_once_with(
            str(mock_uuid4.return_value))
        mock_check_ips_info.assert_called_once_with(
        ...
        mock_write_static_ip_script.assert_called_once_with(
            mock_get_cbslinit_base_dir.return_value,
            mock.sentinel.nics_info,
            mock_compile_static_ip_conf_from_registry.return_value)
        mock_unload_registry_hive.assert_called_once_with(
            "HKLM\\%s" % mock_uuid4.return_value)
