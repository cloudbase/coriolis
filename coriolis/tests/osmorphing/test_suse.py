...
class BaseSUSEMorphingToolsTestCase(test_base.CoriolisBaseTestCase):
    ...
    @mock.patch.object(
        base.BaseLinuxOSMorphingTools, 'get_required_detected_os_info_fields'
    )
    def test_get_required_detected_os_info_fields(
            self, mock_get_required_detected_os_info_fields):
        mock_get_required_detected_os_info_fields.return_value = [
            mock.sentinel.field1, mock.sentinel.field2
        ]
        ...
        mock_get_required_detected_os_info_fields.assert_called_once_with()
    ...
    def test_check_os_supported(self, mock_version_supported_util):
        ...
        mock_version_supported_util.assert_called_once_with(
            self.detected_os_info['release_version'], minimum=12)
    ...
    def test_check_os_supported_opensuse_tumbleweed(
    ...
    def test_check_os_supported_opensuse_unsupported_version(
            self, mock_version_supported_util):
        self.detected_os_info[
            'distribution_name'] = suse.OPENSUSE_DISTRO_IDENTIFIER
        ...
        mock_version_supported_util.assert_called_once_with(
            self.detected_os_info['release_version'], minimum=15)
    ...
    def test__run_mkinitrd_success(self, mock_exec_cmd_chroot):
        ...
        self.event_manager.progress_update.assert_has_calls([
            mock.call("Rebuilding initrds"),
            mock.call("Successfully rebuilt initrds")
        ])
    ...
    def test__run_mkinitrd_with_exception(self, mock_exec_cmd_chroot):
        ...
        self.event_manager.progress_update.assert_has_calls([
            mock.call("Rebuilding initrds"),
            mock.call("Error occurred while rebuilding initrds, skipping")
        ])
    ...
    def test__rebuild_initrds(self, mock_run_dracut, mock_run_mkinitrd,
                              mock_version_supported_util):
        mock_version_supported_util.return_value = True
    ...
    @mock.patch.object(base.BaseLinuxOSMorphingTools, '_exec_cmd_chroot')
    @mock.patch.object(suse.re, 'search')
    def test__enable_sles_module(self, mock_search, mock_exec_cmd_chroot):
        self.morphing_tools._enable_sles_module(mock.sentinel.module)

        mock_exec_cmd_chroot.assert_has_calls([
            mock.call("SUSEConnect --list-extensions").decode(),
    ...
    @mock.patch.object(base.BaseLinuxOSMorphingTools, '_exec_cmd_chroot')
    @mock.patch.object(suse.re, 'search')
    def test__enable_sles_module_with_exception(self, mock_search,
                                                mock_exec_cmd_chroot):
    ...
    def test__add_repo_new(self, mock_get_repos, mock_exec_cmd_chroot,
        ...
        mock_get_repos.assert_called_once()
        self.event_manager.progress_update.assert_called_once_with(
            "Adding repository: alias%s" % mock_uuid4.return_value)
    ...
    def test_uninstall_packages(self, mock_exec_cmd_chroot):
        ...
        self.event_manager.progress_update.assert_not_called()
