...
from coriolis.osmorphing import base, ubuntu
...
class BaseUbuntuMorphingToolsTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test__set_netplan_ethernet_configs(
        self, mock_list_dir, mock_read_file, mock_write_file_sudo, mock_exec_cmd
    ):
        ...
        config_data = {
            'network': {
                'ethernets': {'test0': {'dhcp4': True, 'dhcp6': True}},
                'version': 2,
            }
        }
        ...
        mock_exec_cmd.assert_called_once_with(
            ...
            "'%s/etc/netplan/file2.yaml.bak'" % (self.os_root_dir, self.os_root_dir)
        )
    ...
    def test__set_netplan_ethernet_configs_no_network_data(
        ...
        with self.assertLogs('coriolis.osmorphing.ubuntu', level=logging.DEBUG):
            self.morphing_tools._set_netplan_ethernet_configs(mock.sentinel.nics_info)

        mock_read_file.assert_called_once_with('etc/netplan/file1.yaml')
        mock_exec_cmd.assert_called_once_with(
            "sudo cp '%s/etc/netplan/file1.yaml' "
            "'%s/etc/netplan/file1.yaml.bak'" % (self.os_root_dir, self.os_root_dir)
        )
