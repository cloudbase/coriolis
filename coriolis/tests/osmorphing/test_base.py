...
class BaseOSMorphingToolsTestBase(test_base.CoriolisBaseTestCase):
    ...
    def setUp(self, mock_get_required_fields):
        ...
        self.os_root_dir = mock.sentinel.os_root_dir
        self.os_root_device = mock.sentinel.os_root_device
        self.hypervisor = mock.sentinel.hypervisor
        self.event_manager = mock.sentinel.event_manager
    ...
    def test_get_required_detected_os_info_fields(self):
        self.assertRaises(
            NotImplementedError,
            self.os_morphing_tools.get_required_detected_os_info_fields
        )
    ...
    def test_check_detected_os_info_parameters(self, mock_get_required_fields):
        ...
        result = self.os_morphing_tools.check_detected_os_info_parameters(
            self.detected_os_info)
    ...
    def test_check_detected_os_info_parameters_missing_os_info_fields(
        ...
        self.assertRaises(
            ...
            self.os_morphing_tools.check_detected_os_info_parameters,
            self.detected_os_info
        )
        mock_get_required_fields.assert_called_once_with()
    ...
    def test_check_os_supported_not_implemented(self):
        self.assertRaises(
            NotImplementedError,
            self.os_morphing_tools.check_os_supported,
            self.detected_os_info
        )
...
class BaseLinuxOSMorphingToolsTestBase(test_base.CoriolisBaseTestCase):
    ...
    @ddt.data(
        ("not a float", 1.0, False, logging.WARN),
        ("no match", 1.0, False, logging.WARN),
    )
    @ddt.unpack
    def test_version_supported_util_warnings(
            self, version, minimum, expected_result, expected_log_level):
        if version == "no match":
            ...
            mock_match.groups.return_value = [version]
            with mock.patch("re.match", return_value=mock_match):
                with self.assertLogs('coriolis.osmorphing.base',
                                     level=expected_log_level):
                    result = self.os_morphing_tools._version_supported_util(
                        version, minimum)
    ...
    @mock.patch('os.path')
    def test__test_path(self, mock_os_path, mock_test_ssh_path):
        result = self.os_morphing_tools._test_path(mock.sentinel.path)

        mock_test_ssh_path.assert_called_once_with(
            self.os_morphing_tools._ssh, mock_os_path.join.return_value)
        self.assertEqual(result, mock_test_ssh_path.return_value)

    @mock.patch.object(base.utils, 'read_ssh_file')
    @mock.patch('os.path')
    def test__read_file(self, mock_os_path, mock_read_ssh_file):
        result = self.os_morphing_tools._read_file(mock.sentinel.path)
    ...
    @mock.patch.object(base.utils, 'write_ssh_file')
    @mock.patch('os.path')
    def test__write_file(self, mock_os_path, mock_write_ssh_file):
        self.os_morphing_tools._write_file(
            mock.sentinel.path, mock.sentinel.content)
    ...
    @mock.patch('os.path')
    def test__list_dir(self, mock_os_path, mock_list_ssh_dir):
        result = self.os_morphing_tools._list_dir(mock.sentinel.path)
    ...
    @mock.patch('os.path')
    def test__read_config_file(self, mock_os_path, mock_read_ssh_ini):
        result = self.os_morphing_tools._read_config_file(
            mock.sentinel.path, check_exists=False)
    ...
    @mock.patch('os.path')
    def test__copy_resolv_conf(self, mock_os_path, mock_exec_cmd,
                               mock_test_path):
        resolv_conf_path_old = "%s.old" % mock_os_path.join.return_value
        mock_test_path.return_value = True

        self.os_morphing_tools._copy_resolv_conf()

        mock_test_path.assert_called_once_with(mock_os_path.join.return_value)
        mock_exec_cmd.assert_has_calls([
            mock.call('sudo mv -f %s %s' % (
                mock_os_path.join.return_value, resolv_conf_path_old)),
    ...
    @mock.patch.object(base.BaseLinuxOSMorphingTools, '_exec_cmd')
    @mock.patch('os.path')
    def test__restore_resolv_conf(self, mock_os_path, mock_exec_cmd,
                                  mock_test_path):
        resolv_conf_path_old = "%s.old" % mock_os_path.join.return_value
        mock_test_path.return_value = True
        ...
        mock_exec_cmd.assert_called_once_with(
            'sudo mv -f %s %s' % (resolv_conf_path_old,
                                  mock_os_path.join.return_value))
    ...
    def test__replace_fstab_entries_device_prefix(
        ...
        self.os_morphing_tools._event_manager = mock.MagicMock()
        ...
        self.os_morphing_tools._event_manager.progress_update.\
            assert_called_once_with(
                "Replacing all /etc/fstab entries prefixed with '%s' to '%s'" %
                (current_prefix, new_prefix))
        mock_exec_cmd_chroot.assert_called_once_with(
    ...
    @mock.patch.object(base.BaseLinuxOSMorphingTools, '_exec_cmd_chroot')
    @mock.patch.object(
        base.BaseLinuxOSMorphingTools, '_validate_grub_config_obj'
    )
    def test_set_grub_value_append(self, mock_validate_grub_config_obj,
                                   mock_exec_cmd_chroot, mock_read_file_sudo):
        option = 'option'
        value = 'value'
        config_obj = {
        ...
        mock_validate_grub_config_obj.assert_called_once_with(config_obj)
        mock_exec_cmd_chroot.assert_called_once_with(
            'sed -ie \'$a%s="%s"\' %s' % (
    ...
    def test_set_grub_value_replace(self, mock_validate_grub_config_obj,
                                    mock_exec_cmd_chroot, mock_read_file_sudo):
        option = 'option'
        value = 'value'
        config_obj = {
    ...
    def test__apply_grub2_config(self, mock_validate_grub_config_obj,
                                 mock_exec_cmd_chroot,
    ...
    def test__apply_grub2_config_no_update_grub(
            self, mock_validate_grub_config_obj, mock_exec_cmd_chroot,
            mock_execute_update_grub):
        config_obj = {
            'location': mock.sentinel.location,
            'source': mock.sentinel.source,
