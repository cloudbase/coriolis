...
class BaseOpenWRTMorphingToolsTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test_check_os_supported(self):
        detected_os_info = {'distribution_name': openwrt.OPENWRT_DISTRO_IDENTIFIER}
        result = openwrt.BaseOpenWRTMorphingTools.check_os_supported(detected_os_info)

        self.assertTrue(result)

    def test_check_os_not_supported(self):
        detected_os_info = {'distribution_name': 'unsupported'}
        result = openwrt.BaseOpenWRTMorphingTools.check_os_supported(detected_os_info)
    ...
    def test_get_update_grub2_command(self):
        ...
        self.assertRaises(NotImplementedError, morphing_tools.get_update_grub2_command)
