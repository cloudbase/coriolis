...
class BaseRedHatMorphingToolsTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test_check_os_supported(self, mock_version_supported_util):
        result = self.morphing_tools.check_os_supported(
            self.detected_os_info)

        mock_version_supported_util.assert_called_once_with(
            self.detected_os_info['release_version'], minimum=6)
    ...
    @mock.patch.object(redhat.utils, 'get_udev_net_rules')
    def test_add_net_udev_rules(self, mock_get_udev_net_rules,
                                mock_write_file_sudo, mock_test_path):
        mock_test_path.return_value = False

        self.morphing_tools._add_net_udev_rules(mock.sentinel.net_ifaces_info)

        mock_get_udev_net_rules.assert_called_once_with(
            mock.sentinel.net_ifaces_info)
        mock_write_file_sudo.assert_called_once_with(
            "etc/udev/rules.d/70-persistent-net.rules",
            mock_get_udev_net_rules.return_value
        )
    ...
    def test_write_nic_configs(
        ...
        nics_info = [mock.MagicMock(), mock.MagicMock()]
        mock_test_path.return_value = True

        self.morphing_tools._write_nic_configs(nics_info)

        mock_write_file_sudo.assert_has_calls([
            mock.call("etc/sysconfig/network-scripts/ifcfg-eth0", mock.ANY),
            mock.call("etc/sysconfig/network-scripts/ifcfg-eth1", mock.ANY)
        ])
        mock_exec_cmd_chroot.assert_has_calls([
