...
class BaseDebianMorphingToolsTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test_check_os_supported(self, mock_version_supported_util):
        result = self.morpher.check_os_supported(
            self.detected_os_info)

        mock_version_supported_util.assert_called_once_with(
            self.detected_os_info['release_version'], minimum=8)
    ...
    @mock.patch('coriolis.osmorphing.debian.StringIO')
    def test__compose_interfaces_config(self, mock_stringio):
        mock_stringio_instance = mock_stringio.return_value
        result = self.morpher._compose_interfaces_config(self.nics_info)

        mock_stringio_instance.write.assert_has_calls([
            mock.call(debian.LO_NIC_TPL),
            mock.call("\n\n"),
            mock.call(debian.INTERFACES_NIC_TPL % {"device_name": "eth0"}),
            mock.call("\n\n"),
            mock.call(debian.INTERFACES_NIC_TPL % {"device_name": "eth1"}),
            mock.call("\n\n")
        ])
        mock_stringio_instance.seek.assert_called_once_with(0)

        self.assertEqual(result, mock_stringio_instance.read.return_value)

    @mock.patch('yaml.dump')
    def test__compose_netplan_cfg(self, mock_yaml_dump):
        expected_cfg = {
            "network": {
                "version": 2,
                "ethernets": {
        ...
        self.assertEqual(result, mock_yaml_dump.return_value)

    @mock.patch.object(debian.BaseDebianMorphingTools, '_write_file_sudo')
    @mock.patch.object(debian.BaseDebianMorphingTools, '_exec_cmd_chroot')
    @mock.patch.object(debian.BaseDebianMorphingTools, '_test_path')
    @mock.patch.object(debian.BaseDebianMorphingTools, '_compose_netplan_cfg')
    @mock.patch.object(debian.BaseDebianMorphingTools, '_list_dir')
    @mock.patch.object(
        debian.BaseDebianMorphingTools, 'disable_predictable_nic_names'
    )
    @mock.patch.object(
        debian.BaseDebianMorphingTools, '_compose_interfaces_config'
    )
    def test_set_net_config(
            self, mock_compose_interfaces_config,
            mock_disable_predictable_nic_names, mock_list_dir,
            mock_compose_netplan_cfg, mock_test_path, mock_exec_cmd_chroot,
        ...
        mock_compose_netplan_cfg.assert_called_once_with(self.nics_info)
        mock_compose_interfaces_config.assert_called_once_with(self.nics_info)
        mock_write_file_sudo.assert_has_calls(
            [mock.call('etc/network/interfaces',
                       mock_compose_interfaces_config.return_value),
             mock.call('etc/netplan/coriolis_netplan.yaml',
                       mock_compose_netplan_cfg.return_value)])
    ...
    def test_set_net_config_no_dhcp(
        ...
        mock_compose_netplan_cfg.assert_not_called()
        mock_compose_interfaces_config.assert_not_called()
        mock_write_file_sudo.assert_not_called()
        mock_exec_cmd_chroot.assert_not_called()
    ...
    def test_pre_packages_install(self, mock_exec_cmd_chroot,
        ...
        self._event_manager.progress_update.assert_called_once_with(
    ...
    def test_install_packages(self, mock_exec_cmd_chroot):
        ...
        mock_exec_cmd_chroot.assert_called_once_with(apt_get_cmd)
