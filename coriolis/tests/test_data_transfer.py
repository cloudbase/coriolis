...
from coriolis import data_transfer, exception
...
class DataTranferTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test_get_session_and_address(self, mock_conf):
        ...
        self.assertEqual(result[1], "http://%s/" % mock_conf.compressor_address)
    ...
    def test_get_session_and_address_unix_socket(self, mock_conf, mock_stat):
        ...
        self.assertEqual(
            result[1],
            "http+unix://%s/" % parse.quote_plus(mock_conf.compressor_address),
        )
    ...
    def test_get_session_and_address_nonexistent_path(self, mock_conf):
        ...
        self.assertRaises(
            exception.CoriolisException, data_transfer._get_session_and_address
        )

    @mock.patch.object(data_transfer.os, 'stat')
    @mock.patch.object(data_transfer, 'CONF')
    def test_get_session_and_address_invalid_unix_socket(self, mock_conf, mock_stat):
        mock_conf.compressor_address = '/var/run/compressor.sock'
    ...
    def test_compression_proxy(self, mock_compress_func):
        ...
        self.assertEqual(result, (mock_compress_func[self.fmt].return_value, True))

    @mock.patch.object(data_transfer, '_get_session_and_address')
    @mock.patch.object(data_transfer, 'CONF')
    def test_compression_proxy_with_compression(
        self, mock_conf, mock_get_session_and_address
    ):
        mock_get_session_and_address.return_value = (self.mock_session, self.mock_url)
        result = data_transfer.compression_proxy(self.data_content, self.fmt)
        ...
        self.assertEqual(result, (self.mock_session.post.return_value.content, False))
        self.mock_session.close.assert_called_once()

    @mock.patch.object(data_transfer, '_get_session_and_address')
    @mock.patch.object(data_transfer, '_COMPRESS_FUNC')
    def test_compression_proxy_with_exception(
        self, mock_compress_func, mock_get_session_and_address
    ):
        mock_get_session_and_address.return_value = (self.mock_session, self.mock_url)
    ...
    def test_encode_data(self, mock_struct, mock_compression_proxy):
        ...
        mock_struct.pack.side_effect = lambda fmt, *args: struct.pack(fmt, *args)

        result = data_transfer.encode_data(
            self.msg_id, self.path, self.offset, self.data_content, True
        )
        expected_result = struct.pack(
            '<III',
            self.msg_id,
            len(self.data_content),
            len(self.path) + 1 + 8 + len(self.data_content),
        )
        expected_content = self.path.encode() + b'\0' + struct.pack("<Q", self.offset)
    ...
    def test_encode_data_uncompressed(self, mock_struct):
        ...
        result = data_transfer.encode_data(
            self.msg_id, self.path, self.offset, self.data_content, False
        )
        inflated_content = (
            self.path.encode()
            + b'\0'
