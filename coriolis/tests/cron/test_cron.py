...
class CronJobTestCase(test_base.CoriolisBaseTestCase):
    ...
    def setUp(self, *_):
        ...
        args = ['arg1', 'arg2']
        kw = {"arg3": "mock_arg3", "arg4": "mock_arg4"}
    ...
    @ddt.data(
        {'pairs': [("mock_field1", "mock_field2")], 'expected_result': [False]},
        {'pairs': [("mock_field1", "mock_field1")], 'expected_result': [True]},
        {'pairs': [("mock_field1", None)], 'expected_result': [True]},
        {'pairs': [(None, "mock_field2")], 'expected_result': [False]},
        {'pairs': [(None, None)], 'expected_result': [True]},
    )
    ...
    @ddt.data(
        ...
        {'schedule': {'month': 2, 'hour': 0}, 'expected_result': False},
        {'schedule': {'month': 1, 'hour': 1}, 'expected_result': False},
    )
    ...
    def test_should_run_false(self, mock_is_expired, mock_compare):
        ...
        dt = datetime.datetime.fromisoformat('2099-01-01')
        self.cron._enabled = True
        self.cron.schedule = {'month': 1, 'hour': 0}

        self.assertRaises(exception.CoriolisException, self.cron.should_run, None)
    ...
    def test_start(self, mock_send_status):
        ...
        self.cron._func.assert_called_once_with(*self.cron._args, **self.cron._kw)
        self.cron._on_success.assert_called_once_with(self.cron._func.return_value)
    ...
    @mock.patch.object(cron.CronJob, '_send_status')
    def test_start_on_error(self, mock_send_status, mock_exc_info, mock_LOG):
...
class CronTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test_register(self, *_):
        ...
        self.assertEqual(self.cron._jobs[mock.sentinel.name], job)

    def test_register_no_job(self):
        self.assertRaises(ValueError, self.cron.register, None)

    def test_unregister(self):
        self.cron._jobs = {'job_1': 'mock_job', 'job_2': 'mock_job'}
        self.cron.unregister('job_1')
        self.assertEqual({'job_2': 'mock_job'}, self.cron._jobs)
    ...
    def test_unregister_jobs_with_prefix(self):
        ...
        self.assertEqual({'pre2_job_3': 'mock_job'}, self.cron._jobs)
    ...
    def test_check_jobs(self, mock_spawn):
        ...
        mock_spawn.assert_called_once_with(mock_job.start, args=[self.cron._queue])
    ...
    def test_loop(self, mock_run_pending, mock_sleep):
        ...
        self.assertRaises(CoriolisTestException, self.cron._loop)
    ...
    def test_result_loop(self, mock_LOG):
        ...
        self.assertRaises(CoriolisTestException, self.cron._result_loop)
    ...
    def test_janitor(self, mock_sleep):
        ...
        self.assertRaises(CoriolisTestException, self.cron._janitor)

        self.assertEqual({'job2': job2}, self.cron._jobs)

    @mock.patch('coriolis.utils.start_thread')
    @mock.patch.object(schedule, 'every')
    def test_start(self, mock_every, mock_spawn):
        mock_spawn.side_effect = ['spawn_loop', 'spawn_janitor', 'spawn_result_loop']

        self.cron.start()

        mock_every.return_value.minute.do.assert_called_once_with(self.cron._check_jobs)
        ...
        self.assertEqual(
            ['spawn_loop', 'spawn_janitor', 'spawn_result_loop'], self.cron._threads
        )

    def test_stop(self):
        self.cron._should_stop = False
        self.cron.stop()
        self.assertEqual(True, self.cron._should_stop)
