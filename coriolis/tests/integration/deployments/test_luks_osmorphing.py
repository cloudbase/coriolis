...
class _LUKSOSMorphingMixin:
    ...
    def _prepare_src_device(self):
        ...
        with osmorphing_utils.luks_open(
            self._src_device, self._key_file, disable_keyring=True
        ) as mapper:
    ...
    def test_deployment_with_os_morphing(self):
        ...
        self.assertTrue(
            osmorphing_utils.luks_can_open(self._dst_device, self._migration_key_file),
        ...
        keyfile_content = osmorphing_utils.read_file_from_luks_device(
            self._dst_device, self._key_file, "etc/luks/coriolis_%s.key" % dst_basename
        )
        self.assertEqual(
            keyfile_content.strip(),
            _MIGRATION_PASSPHRASE,
        ...
        self.assertFalse(
            osmorphing_utils.luks_can_open(self._dst_device, self._tpm_key_file),
        ...
        self.assertIsNotNone(crypttab, "/etc/crypttab not found on destination")
        self.assertNotIn(
            "tpm2-",
            crypttab,
