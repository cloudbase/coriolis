...
class WSManConnectionTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test_from_connection_info(self, mock_wait_for_port_connectivity, mock_connect):
        connection_info = {
            "ip": "127.0.0.1",
            "username": "user",
            "password": "pass",
        ...
        mock_wait_for_port_connectivity.assert_called_once_with("127.0.0.1", 5986)
    ...
    def test_from_connection_info_missing_keys(self):
        self.assertRaises(
            ValueError,
            self.conn.from_connection_info,
            {"username": "user", "password": "pass"},
        )

    def test_from_connection_info_invalid_type(self):
        self.assertRaises(
            ValueError, self.conn.from_connection_info, 'invalid-connection-type'
        )
    ...
    def test_set_timeout(self):
        ...
        self.assertEqual(self.conn._protocol.transport.timeout, self.conn._conn_timeout)
        self.assertEqual(self.conn._protocol.timeout, self.conn._conn_timeout)

    def test__exec_command(self):
        self.conn._protocol.get_command_output.return_value = ("std_out", "std_err", 0)
    ...
    def test__exec_command_exception(self):
        self.conn._protocol.get_command_output.side_effect = (
        ...
        self.conn._protocol.cleanup_command.assert_called_once_with(mock.ANY, mock.ANY)
    ...
    def test__exec_command_invalid_credentials(self, mock_sleep):
        ...
        self.assertRaises(
            exception.NotAuthorized, self.conn._exec_command, self.cmd, self.args
        )
    ...
    def test_exec_command(self):
        self.conn._protocol.get_command_output.return_value = ("std_out", "std_err", 0)
        exp_sanitized_log = (
            "DEBUG:coriolis.wsman:Executing WSMAN command: %s" % self.sanitized_cmd
        )
    ...
    def test_exec_command_exception(self):
        self.conn._protocol.get_command_output.return_value = ("std_out", "std_err", 1)
        self.assertRaises(
