...
class EventManagerTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test_call_event_handler_method_not_exist(self):
        ...
        self.assertRaises(
            AttributeError,
            event_manager._call_event_handler,
            'non_existent_method',
            'arg1',
            'arg2',
            kwarg1='value1',
    ...
    def test_add_percentage_step(self, test_data):
        ...
        if 'expected_log_level' in test_data:
            with self.assertLogs(
                'coriolis.events', level=test_data['expected_log_level']
            ):
            ...
            self.assertEqual(perc_step_data.last_perc, test_data['expected_perc'])
            self.assertEqual(
                perc_step_data.last_value, test_data['expected_initial_step']
            )
            self.assertEqual(
                perc_step_data.total_steps, test_data['expected_total_steps']
            )
    ...
    def test_set_percentage_step(self):
        ...
        self.event_manager._perc_steps[mock.sentinel.progress_update_id] = (
            perc_step_data
        )
    ...
    def test_set_percentage_step_no_perc_step(self):
        self.event_manager.set_percentage_step(
            events._PercStepData(mock.sentinel.progress_update_id, 60, 0, 100), 60
        )
    ...
    def test_set_percentage_step_rollback(self):
        ...
        with self.assertLogs('coriolis.events', level=logging.WARN):
            result = self.event_manager.set_percentage_step(
                events._PercStepData(mock.sentinel.progress_update_id, 60, 0, 100), 40
            )
        self.assertEqual(result, None)
