...
class BaseCoriolisTaskflowTaskTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test_get_error_str_for_flow_no_failures(self):
        result = self.task._get_error_str_for_flow_failures(None, full_tracebacks=False)
...
class BaseRunWorkerTaskTestCase(test_base.CoriolisBaseTestCase):
    ...
    def setUp(self):
        ...
        self.task = base.BaseRunWorkerTask(
            'test_task', 'test_task_id', 'test-task_instance', 'test_runner_type'
        )
    ...
    def test_scheduler_client_property(self):
        ...
        with mock.patch(
            'coriolis.scheduler.rpc.client.SchedulerClient',
            return_value=mock_scheduler_client,
        ):
            self.assertEqual(self.task._scheduler_client, mock_scheduler_client)
    ...
    def test_set_provides_for_dependencies(self):
        ...
        self.assertEqual(
            self.kwargs,
            {'test_task_id': 'test_task_id', 'provides': ['task-test_task-result']},
        )
    ...
    def test_set_provides_for_dependencies_with_provides_key(self):
        ...
        self.assertEqual(kwargs, {'provides': ['test', 'task-test_task-result']})

    def test_set_requires_for_dependencies(self):
        self.task._set_requires_for_dependencies(self.kwargs, self.depends_on)
        self.assertEqual(
            self.kwargs,
            {'test_task_id': 'test_task_id', 'requires': ['task-test_task_id-result']},
        )
    ...
    def test_set_requires_for_dependencies_with_requires_key(self):
        ...
        self.assertEqual(kwargs, {'requires': ['test', 'task-test_task_id-result']})
    ...
    def test_set_requires_for_task_info_fields(self, mock_get_task_runner):
        self.mock_task_runner.get_required_task_info_properties.return_value = ['prop1']
    ...
    @mock.patch('coriolis.tasks.factory.get_task_runner_class')
    def test_set_requires_for_task_info_fields_with_cleanup(self, mock_get_task_runner):
        self.mock_task_runner.get_required_task_info_properties.return_value = [
            'main_task_dep1',
            'main_task_dep2',
        ]
        self.mock_task_runner.get_returned_task_info_properties.return_value = [
            'main_task_dep1'
        ]

        self.mock_cleanup_task_runner.get_required_task_info_properties.return_value = [
            'cleanup_task_dep1',
            'cleanup_task_dep2',
        ]

        mock_get_task_runner.side_effect = [
            self.mock_task_runner,
    ...
    def test_set_provides_for_task_info_fields(self, mock_get_task_runner):
        self.mock_task_runner.get_returned_task_info_properties.return_value = ['prop1']
        mock_get_task_runner.return_value = self.mock_task_runner
        kwargs = {'provides': ['test']}
    ...
    @mock.patch('coriolis.tasks.factory.get_task_runner_class')
    def test_set_provides_for_task_info_fields_with_cleanup(self, mock_get_task_runner):
        self.mock_task_runner.get_returned_task_info_properties.return_value = [
            'main_task_dep1',
            'main_task_dep2',
        ]
        ...
        self.mock_cleanup_task_runner.get_returned_task_info_properties.return_value = [
            'cleanup_task_dep1',
            'cleanup_task_dep2',
        ]
    ...
    def test_get_worker_service_rpc_for_task(
        self, mock_get_worker_service, mock_from_service_definition
    ):
    ...
    @mock.patch.object(base.BaseRunWorkerTask, '_get_worker_service_rpc_for_task')
    def test_execute_task(self, mock_get_worker_rpc):
        mock_get_worker_rpc.return_value = self.mock_worker_rpc

        result = self.task._execute_task(
            'ctxt', 'task_id', 'task_type', 'origin', 'destination', 'task_info'
        )
    ...
    def test_execute(self, mock_execute_task):
        result = self.task.execute('ctxt', 'origin', 'destination', 'task_info')
