...
def upgrade(migrate_engine):
    ...
    replica_schedule = sqlalchemy.Table('replica_schedules', meta, autoload=True)
    ...
    with migrate_engine.connect() as conn:
        conn.execute(
            'UPDATE base_transfer_action SET type = "transfer" WHERE type = "replica";'
        )
        conn.execute(
            'UPDATE tasks_execution SET type = "transfer_execution"'
