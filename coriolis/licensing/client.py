...
class LicensingClient(object):
    ...
    def from_env(cls):
        ...
        if base_url in ["", None, "None", "null"]:
            ...
            return None
        allow_untrusted = (
            os.environ.get("LICENSING_SERVER_ALLOW_UNTRUSTED", None) is not None
        )
        client = cls(base_url, appliance_id=None, allow_untrusted=allow_untrusted)
        ...
        if not appliance_ids:
            ...
            raise exception.CoriolisException('More than one appliance IDs found.')
    ...
    def _do_req(
        ...
        kwargs = {"verify": self._verify, "timeout": CONF.default_requests_timeout}
        if body:
    ...
    def get_appliance(self):
        ...
        return self._get(
            "/appliances/%s" % self._appliance_id,
    ...
    def add_reservation(self, reservation_type, num_vms):
        ...
        allowed_values = [RESERVATION_TYPE_MIGRATION, RESERVATION_TYPE_REPLICA]
        if reservation_type not in allowed_values:
            raise ValueError("Reservation type must be one of %s" % allowed_values)
        return self._post(
            "/reservations",
    ...
    def check_refresh_reservation(self, reservation_id):
        ...
        return self._post(
            "/reservations/%s/refresh" % reservation_id,
            None,
            response_key="reservation",
        )
    ...
    def mark_reservation_fulfilled(self, reservation_id):
        ...
        return self._post(
            "/reservations/%s/fulfill" % reservation_id,
            None,
            response_key="reservation",
        )
    ...
    def delete_reservation(self, reservation_id, raise_on_404=False):
        ...
        if not resp.ok:
            if resp.status_code == 404:
                if raise_on_404:
                    self._raise_response_error(resp)
                LOG.warn("Got 404 when deleting reservation '%s'", reservation_id)
            else:
                self._raise_response_error(resp)
