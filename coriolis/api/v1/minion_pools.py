...
class MinionPoolController(api_wsgi.Controller):
    ...
    def _validate_create_body(self, ctxt, body):
        ...
        if pool_os_type not in constants.VALID_OS_TYPES:
            raise Exception(
                ...
                "of the following: %s" % (pool_os_type, constants.VALID_OS_TYPES)
            )
        ...
        if pool_platform not in supported_pool_platforms:
            raise Exception(
                ...
                " of the following: %s" % (pool_platform, supported_pool_platforms)
            )
        if pool_platform == constants.PROVIDER_PLATFORM_SOURCE and (
            ...
            raise Exception(
                ...
                "'%s', not '%s'." % (constants.OS_TYPE_LINUX, pool_os_type)
            )
        ...
        maximum_minions = minion_pool.get("maximum_minions", minimum_minions)
        ...
        self._check_pool_retention_strategy(minion_retention_strategy)
    ...
    def _validate_updated_environment_options(
        self, context, minion_pool, environment_options
    ):
        if minion_pool['platform'] == (constants.PROVIDER_PLATFORM_SOURCE):
            (self._endpoints_api.validate_endpoint_source_minion_pool_options)(
                context, minion_pool['endpoint_id'], environment_options
            )
        elif minion_pool['platform'] == (constants.PROVIDER_PLATFORM_DESTINATION):
            (self._endpoints_api.validate_endpoint_destination_minion_pool_options)(
                context, minion_pool['endpoint_id'], environment_options
            )
        else:
            raise Exception("Unknown pool platform: %s" % minion_pool['platform'])
    ...
    def _validate_update_body(self, id, context, body):
        ...
        if 'endpoint_id' in minion_pool:
            raise Exception("The 'endpoint_id' of a minion pool cannot be updated.")
        if 'platform' in minion_pool:
            raise Exception("The 'platform' of a minion pool cannot be updated.")
        vals = {
            k: minion_pool[k]
            for k in minion_pool.keys()
            & {
                "name",
                "environment_options",
                "minimum_minions",
        ...
        if 'minion_retention_strategy' in vals:
            self._check_pool_retention_strategy(vals['minion_retention_strategy'])
        if any(
            [
                f in vals
                for f in [
            ...
            minion_pool = self._minion_pool_api.get_minion_pool(context, id)
            self._check_pool_numeric_values(
                vals.get('minimum_minions', minion_pool['minimum_minions']),
                vals.get('maximum_minions', minion_pool['maximum_minions']),
